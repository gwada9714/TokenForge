...
vi.mock("../../src/components/features/launchpad/LaunchpadForm", () => ({
  LaunchpadForm: vi
    .fn()
    .mockImplementation(({ contractAddress }) => (
...
describe("LaunchpadPage Component", () => {
  ...
  it("renders the component with network connected", () => {
    ...
    expect(
      screen.getByText(/Plateforme de Lancement et Gestion de Liquidité/i)
    ).toBeInTheDocument();
  ...
  it("shows a warning when wallet is not connected", () => {
    ...
    expect(
      screen.getByText(/Please connect your wallet to continue/i)
    ).toBeInTheDocument();
  });
  ...
  it("shows network not supported message on unsupported networks", () => {
    ...
    expect(
      screen.getByText(/Launchpad is not available on Polygon/i)
    ).toBeInTheDocument();
  });
  ...
  it("passes the correct contract address to LaunchpadForm based on network", () => {
    // Test pour Sepolia
    (useNetwork as any).mockReturnValue(
      setupMockNetwork({ chain: { id: 11155111, name: "Sepolia" } })
    );
    ...
    // Test pour Mainnet
    (useNetwork as any).mockReturnValue(
      setupMockNetwork({ chain: { id: 1, name: "Ethereum" } })
    );
    ...
    // Test pour Local
    (useNetwork as any).mockReturnValue(
      setupMockNetwork({ chain: { id: 31337, name: "Localhost" } })
    );
