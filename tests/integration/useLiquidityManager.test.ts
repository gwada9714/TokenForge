...
describe("useLiquidityManager hook", () => {
  beforeEach(() => {
    ...
    (query as any).mockImplementation((collectionRef, ...conditions) => {
      ...
      if (collectionRef.path === "liquiditySettings") {
        return {
          _queryType: "liquiditySettings",
          path: "liquiditySettings",
          conditions,
        };
      }
  ...
  it("should load user tokens and liquidity settings on mount", async () => {
    ...
    expect(result.current.userLiquiditySettings[tokenAddress].enabled).toBe(
      true
    );
    expect(result.current.userLiquiditySettings[tokenAddress].targetRatio).toBe(
      50
    );
  });
  ...
  it("should modify existing liquidity settings", async () => {
    ...
    await act(async () => {
      await result.current.modifyLiquiditySettings(
        tokenAddress,
        updatedSettings
      );
    });
    ...
    expect(
      result.current.userLiquiditySettings[tokenAddress].liquidityPair
    ).toBe("USDT");
  });
