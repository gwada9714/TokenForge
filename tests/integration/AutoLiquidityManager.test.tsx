...
describe('AutoLiquidityManager Component', () => {
  ...
  const setupMockLiquidityHook = (overrides = {}) => {
    ...
    const mockStatus = {
      '0xabc1234567890123456789012345678901234567': {
        ...
        nextScheduledRebalance: new Date(Date.now() + 12 * 3600 * 1000).toISOString(),
  ...
  it('shows token details when selecting a token', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText(/Prochaine rebalance prévue/i)).toBeInTheDocument();
    });
  });
  ...
  it('can trigger manual rebalance', async () => {
    ...
    await waitFor(() => {
      expect(mockHook.triggerRebalance).toHaveBeenCalledWith('0xabc1234567890123456789012345678901234567');
    });
  });
  ...
  it('can setup new liquidity settings for a token', async () => {
    ...
    const setupButton = screen.getByText(/Configurer la liquidité automatique/i);
    fireEvent.click(setupButton);
    
    // Remplir le formulaire
    const enabledSwitch = screen.getByLabelText(/Activer la gestion automatique/i);
