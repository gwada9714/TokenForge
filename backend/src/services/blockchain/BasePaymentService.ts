...
export abstract class BasePaymentService implements IPaymentService {
  ...
  async initPaymentSession(params: PaymentInitParams): Promise<PaymentSession> {
    ...
    if (!currency) {
      throw new Error(`Cryptocurrency ${params.currency} not supported on ${this.networkName}`);
    }
  ...
  async confirmPayment(sessionId: string, txHash: string): Promise<boolean> {
    ...
    try {
      ...
      await this.savePaymentError(sessionId, error instanceof Error ? error.message : String(error));
  ...
  async convertEURtoCrypto(amountEUR: number, currencySymbol: string): Promise<CryptoAmount> {
    try {
      ...
      if (!currency) {
        throw new Error(`Currency ${currencySymbol} not supported on ${this.networkName}`);
      }
      ...
      if (currency.isStablecoin) {
        ...
        return {
          amount: Math.floor(adjustedAmount * Math.pow(10, currency.decimals)).toString(),
   ...
   */
  protected async savePaymentSession(session: PaymentSession, params: PaymentInitParams): Promise<void> {
    // Dans une implémentation réelle, on enregistrerait dans Firebase
    console.log(`Saving payment session ${session.sessionId} for ${this.networkName}`);
   ...
   */
  protected async getPaymentSession(sessionId: string): Promise<PaymentSession | null> {
   ...
   */
  protected async updatePaymentStatus(sessionId: string, status: PaymentStatus): Promise<void> {
    // Dans une implémentation réelle, on mettrait à jour dans Firebase
    console.log(`Updating payment session ${sessionId} status to ${status} for ${this.networkName}`);
   ...
   * @param txHash Hash de la transaction
   */
  protected async saveTransactionHash(sessionId: string, txHash: string): Promise<void> {
    // Dans une implémentation réelle, on enregistrerait dans Firebase
    console.log(`Saving transaction hash ${txHash} for session ${sessionId} on ${this.networkName}`);
   ...
   */
  protected async getTransactionHash(sessionId: string): Promise<string | null> {
    // Dans une implémentation réelle, on récupérerait depuis Firebase
    console.log(`Getting transaction hash for session ${sessionId} on ${this.networkName}`);
   ...
   */
  protected async savePaymentError(sessionId: string, error: string): Promise<void> {
    // Dans une implémentation réelle, on enregistrerait dans Firebase
    console.log(`Saving payment error for session ${sessionId} on ${this.networkName}: ${error}`);
