...
export class NotificationService {
  ...
  private channelHandlers: Map<
    NotificationChannel,
    (notification: Notification, userId: string) => Promise<boolean>
  > = new Map();
  ...
  public async notify(
    ...
    if (userPreferences && userPreferences.types[type] === false) {
      logger.debug(
        "NotificationService",
        `Notification de type ${type} désactivée pour l'utilisateur ${userId}`
      );
      return "";
    }
    ...
    if (this.isInDoNotDisturbMode(userId)) {
      logger.debug(
        "NotificationService",
        `Utilisateur ${userId} en mode "Ne pas déranger"`
      );
    ...
    const deliveryPromises = channels.map((channel) =>
      this.deliverToChannel(channel, notification, userId)
    );
  ...
  public async notifyAll(
    ...
    if (userIds.length === 0) {
      logger.warn(
        "NotificationService",
        "Aucun utilisateur disponible pour l'envoi de notification système"
      );
      return 0;
    }
    ...
    logger.info(
      "NotificationService",
      `Notification système envoyée à ${successCount} utilisateurs`,
      {
        type,
        title,
        successCount,
  ...
  public markAsRead(userId: string, notificationId: string): boolean {
    ...
    logger.debug(
      "NotificationService",
      `Notification ${notificationId} marquée comme lue pour l'utilisateur ${userId}`
    );
  ...
  public markAllAsRead(userId: string): number {
    ...
    logger.debug(
      "NotificationService",
      `${count} notifications marquées comme lues pour l'utilisateur ${userId}`
    );
  ...
  public deleteNotification(userId: string, notificationId: string): boolean {
    ...
    logger.debug(
      "NotificationService",
      `Notification ${notificationId} supprimée pour l'utilisateur ${userId}`
    );
  ...
  public deleteAllNotifications(userId: string): number {
    ...
    logger.debug(
      "NotificationService",
      `${count} notifications supprimées pour l'utilisateur ${userId}`
    );
  ...
  public getNotifications(
    ...
    const now = Date.now();
    let filteredNotifications = userNotifications.filter(
  ...
  public setPreferences(
    ...
    const updatedPreferences: NotificationPreferences = {
      ...
      doNotDisturb: {
        enabled:
          ...
          existingPreferences.doNotDisturb?.enabled ??
          false,
        startTime:
          preferences.doNotDisturb?.startTime ??
          existingPreferences.doNotDisturb?.startTime,
        endTime:
          preferences.doNotDisturb?.endTime ??
          existingPreferences.doNotDisturb?.endTime,
        timezone:
          preferences.doNotDisturb?.timezone ??
          existingPreferences.doNotDisturb?.timezone,
      },
    };
    ...
    logger.info(
      "NotificationService",
      `Préférences de notification mises à jour pour l'utilisateur ${userId}`
    );
  }
  ...
  public registerChannelHandler(
    ...
    logger.info(
      "NotificationService",
      `Gestionnaire enregistré pour le canal ${channel}`
    );
  }
  ...
  private initializeDefaultChannelHandlers(): void {
    ...
    this.registerChannelHandler(
      ...
      async (notification, userId) => {
        ...
        logger.debug(
          "NotificationService",
          `Simulation d'envoi d'email à l'utilisateur ${userId}`,
          {
    ...
    this.registerChannelHandler(
      ...
      async (notification, userId) => {
        ...
        logger.debug(
          "NotificationService",
          `Simulation d'envoi de SMS à l'utilisateur ${userId}`,
          {
    ...
    this.registerChannelHandler(
      ...
      async (notification, userId) => {
        ...
        logger.debug(
          "NotificationService",
          `Simulation d'envoi de notification push à l'utilisateur ${userId}`,
          {
    ...
    this.registerChannelHandler(
      ...
      async (notification, userId) => {
        ...
        logger.debug(
          "NotificationService",
          `Simulation d'envoi de webhook pour l'utilisateur ${userId}`,
          {
            payload: {
  ...
  private async deliverToChannel(
    ...
    if (userPreferences && userPreferences.channels[channel] === false) {
      logger.debug(
        "NotificationService",
        `Canal ${channel} désactivé pour l'utilisateur ${userId}`
      );
      return false;
    }
    ...
    if (!handler) {
      logger.warn(
        "NotificationService",
        `Aucun gestionnaire pour le canal ${channel}`
      );
      return false;
    }
    ...
    try {
      ...
      if (result) {
        logger.debug(
          "NotificationService",
          `Notification délivrée via ${channel} à l'utilisateur ${userId}`
        );
      } else {
        logger.warn(
          "NotificationService",
          `Échec de livraison via ${channel} à l'utilisateur ${userId}`
        );
      }
      ...
      logger.error(
        "NotificationService",
        `Erreur lors de la livraison via ${channel} à l'utilisateur ${userId}`,
        error
      );
      return false;
    }
