 ...
 */
const generateDocumentCacheKey = (collectionName: string, docId: string): string => {
 ...
 */
const isCacheValid = <T>(cachedResult: CachedResult<T> | undefined): boolean => {
...
export async function queryOptimized(
  ...
  const executeQuery = async (attempt = 1): Promise<DocumentData[]> => {
    try {
      const { results } = await firestoreService.advancedQuery(collectionName, constraints);
  ...
  if (options.realtime) {
    ...
    // Configurer un nouvel abonnement
    const queryObj = await firestoreService.createQuery(collectionName, constraints);
    
    const unsubscribe = onSnapshot(queryObj, (snapshot: any) => {
 ...
 */
export function invalidateDocumentCache(collectionName: string, docId: string): void {
  const cacheKey = generateDocumentCacheKey(collectionName, docId);
