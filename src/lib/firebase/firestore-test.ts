  ...
  try {
    ...
    const result = await firestoreService.executeTransaction(
      async (transaction) => {
        ...
        const doc1Data = doc1Snapshot.data() as DocumentData & {
          value: number;
        };
        const doc2Data = doc2Snapshot.data() as DocumentData & {
          value: number;
        };
    ...
    const doc1AfterTransaction = await firestoreService.getDocument(
      testCollection,
      docId1
    );
    const doc2AfterTransaction = await firestoreService.getDocument(
      testCollection,
      docId2
    );
...
export async function testFirestoreBatch(): Promise<boolean> {
  try {
    ...
    const allDocsCreated = results.every(
      (doc) => doc !== null && doc.createdInBatch === true
    );
    ...
    const allDocsUpdated = updatedDocs.every(
      (doc) => doc !== null && doc.updated === true
    );
