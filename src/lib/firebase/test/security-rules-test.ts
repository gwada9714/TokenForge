  ...
  static async testAnonymousRead(): Promise<any> {
    try {
      ...
      try {
        ...
        return {
          success: false,
          message: 'ERREUR DE SÉCURITÉ: Lecture anonyme autorisée pour une collection protégée',
          data: {
        ...
        return {
          success: true,
          message: 'Lecture anonyme correctement rejetée pour la collection protégée',
          data: {
            isAnonymous: auth.currentUser?.isAnonymous || false,
      ...
      return {
        success: false,
        message: `Erreur lors du test de lecture anonyme: ${error instanceof Error ? error.message : String(error)}`,
        error: error
      };
    } finally {
  ...
  static async testAnonymousCspViolation(): Promise<any> {
    try {
      ...
      try {
        ...
        return {
          success: false,
          message: 'Échec: Un utilisateur anonyme ne peut pas créer de rapport CSP',
          data: {
            isAnonymous: auth.currentUser?.isAnonymous || false,
            error: error instanceof Error ? error.message : String(error)
          }
      ...
      return {
        success: false,
        message: `Erreur lors du test des violations CSP: ${error instanceof Error ? error.message : String(error)}`,
        error: error
      };
    } finally {
      // Déconnexion pour nettoyer l'état
  ...
  static async testAuthenticatedUserProfile(): Promise<any> {
    try {
      ...
      if (!auth.currentUser) {
        return {
          success: false,
          message: 'Utilisateur non authentifié. Connectez-vous d\'abord pour exécuter ce test.',
          data: {
      ...
      try {
        ...
        return {
          ...
          message: `L'utilisateur ${userExists ? 'peut accéder à' : 'n\'a pas encore'} son profil. ${updateSuccess ? 'Mise à jour réussie.' : ''}`,
          data: {
            userId: userId,
      ...
      return {
        success: false,
        message: `Erreur lors du test de profil utilisateur: ${error instanceof Error ? error.message : String(error)}`,
        error: error
      };
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (!isEmailVerified) {
          return {
            success: true,
            message: 'Email non vérifié: Impossible de créer un wallet (comportement correct)',
            data: {
              userId: userId,
        ...
        if (walletDocs.length > 0) {
          ...
          try {
            ...
            return {
              success: false,
              message: `Échec de création de wallet: ${createError instanceof Error ? createError.message : String(createError)}`,
              data: {
                userId: userId,
                isEmailVerified: isEmailVerified,
                error: createError
      ...
      return {
        success: false,
        message: `Erreur lors du test d'accès aux wallets: ${error instanceof Error ? error.message : String(error)}`,
        error: error
      };
    }
  }
  ...
  static async testOtherUserAccess(): Promise<any> {
    try {
      ...
      try {
        ...
        return {
          success: false,
          message: 'ERREUR DE SÉCURITÉ: Accès au profil d\'un autre utilisateur réussi',
          data: {
        ...
        return {
          success: true,
          message: 'Sécurité fonctionnelle: Accès au profil d\'un autre utilisateur correctement refusé',
          data: {
            currentUserId: auth.currentUser.uid,
            otherUserId: otherUserId,
            error: error instanceof Error ? error.message : String(error)
      ...
      return {
        success: false,
        message: `Erreur lors du test d'accès à d'autres profils: ${error instanceof Error ? error.message : String(error)}`,
        error: error
      };
    }
  }
