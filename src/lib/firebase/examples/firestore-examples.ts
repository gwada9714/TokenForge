...
export async function getUserTransactions(
  ...
  try {
    ...
    const constraints = [
      ...
      where('timestamp', '>', Timestamp.fromDate(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))), // Seulement les 30 derniers jours
    ];
...
export async function createNFTCollection(
  ...
  try {
    ...
    if (nftCount > maxBatchSize) {
      throw new Error(`Impossible de créer plus de ${maxBatchSize} NFTs en une seule opération`);
    }
