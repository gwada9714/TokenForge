...
export const LaunchpadPage: React.FC = () => {
  ...
  const launchpadAddress = chain
    ? chain.id === 11155111
      ? import.meta.env.VITE_LAUNCHPAD_CONTRACT_SEPOLIA
      : chain.id === 1
      ? import.meta.env.VITE_LAUNCHPAD_CONTRACT_MAINNET
      : chain.id === 31337
      ? import.meta.env.VITE_LAUNCHPAD_CONTRACT_LOCAL
      : null
    : null;
    ...
    <Container>
      <Box sx={{ py: 4 }}>
        ...
        <Typography variant="subtitle1" color="text.secondary" gutterBottom>
          Créez un lancement équitable et transparent pour votre token avec
          TokenForge
        </Typography>
        ...
        {activeTab === 1 && <AutoLiquidityManager />}
      </Box>
    </Container>
  );
};
