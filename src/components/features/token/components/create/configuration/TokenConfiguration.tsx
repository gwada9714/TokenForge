...
const TokenConfiguration: React.FC = () => {
  ...
  const handleBasicInfoChange = useCallback(
    (field: keyof TokenConfig, value: string | number) => {
  ...
  const handleFeatureToggle = useCallback(
    (feature: keyof TokenConfig["features"]) => {
      dispatch(
        updateTokenConfig({
          features: {
  ...
  const handleTaxConfigChange = useCallback(
    (taxConfig: TaxConfig) => {
  ...
  const handleMaxLimitsChange = useCallback(
    (maxLimits: MaxLimits) => {
  ...
  const handleLiquidityLockChange = useCallback(
    (liquidityLock: LiquidityLock) => {
      if (typeof liquidityLock.unlockDate === "object") {
        liquidityLock.unlockDate = Math.floor(
          liquidityLock.unlockDate.getTime() / 1000
        );
      }
        ...
        {formFields.map((field) => (
          <Grid item xs={12} sm={6} key={field.field}>
            ...
            <TextField
              ...
              onChange={(e) =>
                handleBasicInfoChange(field.field, e.target.value)
              }
                ...
                <FormControlLabel
                  ...
                  label={
                    <Stack direction="row" spacing={1} alignItems="center">
                      <span>{key}</span>
                      <Tooltip
                        title={`Description de la fonctionnalité ${key}`}
                      >
