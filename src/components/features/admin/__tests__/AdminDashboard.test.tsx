...
vi.mock('../AdminTabs', () => ({
  AdminTabs: ({ value, onChange }: { value: number; onChange: (event: any, newValue: number) => void }) => (
    <div data-testid="admin-tabs">
      <span>Current Tab: {value}</span>
      <button onClick={() => onChange({} as React.SyntheticEvent, 0)}>Tab 0</button>
      <button onClick={() => onChange({} as React.SyntheticEvent, 1)}>Tab 1</button>
      <button onClick={() => onChange({} as React.SyntheticEvent, 2)}>Tab 2</button>
      <button onClick={() => onChange({} as React.SyntheticEvent, 3)}>Tab 3</button>
      <button onClick={() => onChange({} as React.SyntheticEvent, 4)}>Tab 4</button>
    </div>
  )
}));
...
describe('AdminDashboard', () => {
  ...
  it('should render the dashboard with default tab', () => {
    ...
    expect(screen.getByText(/Bienvenue dans le tableau de bord d'administration de TokenForge/)).toBeInTheDocument();
  });
  ...
  it('should switch tabs when clicking on tab buttons', () => {
    ...
    expect(screen.queryByTestId('ownership-management')).not.toBeInTheDocument();
