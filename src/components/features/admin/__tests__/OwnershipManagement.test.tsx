...
describe("OwnershipManagement", () => {
  ...
  it("validates ethereum address format", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Please enter a valid Ethereum address")
      ).toBeFalsy();
  ...
  it("handles transfer ownership successfully", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Ownership transferred successfully")
      ).toBeTruthy();
    });
  ...
  it("handles transfer ownership error", async () => {
    mockContract.transferOwnership.mockRejectedValueOnce(
