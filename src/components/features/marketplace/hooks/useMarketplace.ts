...
import type { MarketplaceItem, MarketplaceFilters, MarketplaceStats } from '../types';
...
export const useMarketplace = () => {
  ...
  const loadItems = useCallback(async (filters?: MarketplaceFilters) => {
    if (!contract) return;
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load marketplace stats');
    }
  }, [contract]);

  const createItem = useCallback(async (item: Omit<MarketplaceItem, 'id' | 'status' | 'createdAt' | 'updatedAt'>) => {
    if (!contract) return;
