...
export class PremiumFeaturesService {
  private static readonly features: PremiumFeature[] = [
    {
      ...
      description:
        "Créez votre propre plateforme de lancement de tokens avec whitelist et vesting.",
      pricing: {
        basePrice: 3, // 3 ETH
    ...
    },
    {
      ...
      description:
        "Plateforme de staking dédiée avec APY personnalisable et périodes de lock.",
      pricing: {
        basePrice: 2, // 2 ETH
  ...
  public static isFeatureAvailable(
    featureId: string,
    userTier: string
  ): boolean {
    const feature = this.getFeatureById(featureId);
