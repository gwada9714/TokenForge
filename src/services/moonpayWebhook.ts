...
export const handleMoonPayWebhook = async (payload: MoonPayWebhookPayload): Promise<void> => {
  try {
    // Vérifier la signature du webhook (à implémenter côté serveur)
    const verifyWebhookSignature = httpsCallable(functions, 'verifyMoonPayWebhookSignature');
