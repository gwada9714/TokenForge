...
export const useService = (): UseServiceReturn => {
  ...
  const getNetworkConfig = useCallback(
    (chainId: number): NetworkConfig | undefined => {
  ...
  const getQuote = useCallback(
    async (type: ServiceType, amount?: number): Promise<ServiceQuote> => {
      if (!chainId) {
  ...
  const configureService = useCallback(
    async (type: ServiceType, config: any) => {
      if (!address || !chainId) {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erreur lors de la configuration du service";
        setError(errorMessage);
        logger.error("Erreur lors de la configuration du service", {
          error: err,
        });
      } finally {
