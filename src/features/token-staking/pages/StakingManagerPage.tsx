...
const StakingManagerPage: React.FC = () => {
  ...
  const [userStakingPositions, setUserStakingPositions] = useState<
    StakingPosition[]
  >([
    {
      id: "101",
  ...
  const handleNewPoolChange =
    (field: keyof typeof newPoolForm) =>
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
      setNewPoolForm({
        ...newPoolForm,
        [field]:
          field === "apy" || field === "lockPeriod"
  ...
  const handleCreatePool = (e: React.FormEvent) => {
    e.preventDefault();
    alert(
      `Création de pool simulée: ${newPoolForm.name} avec APY de ${newPoolForm.apy}%`
    );
    ...
    const newPool: StakingPool = {
      ...
      endDate: newPoolForm.endDate
        ? new Date(newPoolForm.endDate).toISOString()
        : undefined,
    };
  ...
  const handleStakeFormChange =
    (field: keyof typeof stakeForm) =>
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
      setStakeForm({
        ...stakeForm,
  ...
  const handleStake = (e: React.FormEvent) => {
    ...
    alert(
      `Staking simulé: ${stakeForm.amount} ${tokenData.symbol} dans ${selectedPool.name}`
    );
  ...
  const unstake = (positionId: string) => {
    ...
    if (position.status === "locked") {
      alert(
        "Cette position est verrouillée et ne peut pas être retirée avant la fin de la période de verrouillage"
      );
      return;
    }
    ...
    setUserStakingPositions(
      userStakingPositions.filter((pos) => pos.id !== positionId)
    );
  ...
  const totalUserStaked = userStakingPositions
    .reduce(
  ...
  const totalUserRewards = userStakingPositions
    .reduce(
                ...
                {stakingPools.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="mt-2 text-gray-600 dark:text-gray-400">
                      Créez un pool de staking pour permettre aux utilisateurs
                          ...
                          <tr key={pool.id}>
                            ...
                            <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  alert(`Modifier le pool ${pool.name}`)
                                }
                              >
                                Modifier
                              </button>
                      ...
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {userStakingPositions.map((position) => (
                          <tr key={position.id}>
                            ...
                            <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                              {parseInt(position.rewards.replace(/,/g, "")) >
                                0 && (
                                <button
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                ...
                <div className="space-y-4">
                  <div>
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      Total staké (tous pools)
                    </div>
                    <div className="text-2xl font-bold text-gray-900 dark:text-white">
                      {stakingPools
                        .reduce(
                          (sum, pool) =>
                            sum + parseInt(pool.totalStaked.replace(/,/g, "")),
                          0
                        )
                        .toLocaleString()}{" "}
                      {tokenData.symbol}
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                      (
                      {(
                        (parseInt(totalUserStaked.replace(/,/g, "")) /
                  ...
                  <div>
                    ...
                    <div className="text-lg font-medium text-gray-900 dark:text-white">
                      {(
                        stakingPools.reduce((sum, pool) => sum + pool.apy, 0) /
                        stakingPools.length
                      ).toFixed(2)}
                      %
                    </div>
