...
export const TKNStakingPanel: React.FC = () => {
  ...
  const handleStake = async () => {
    ...
    try {
      ...
      const tier = stakingTiers.reduce((highest, tier) => {
        return stakeAmount >= tier.minAmount && tier.id > highest
          ? tier.id
          : highest;
      }, 0);
      ...
      setStakingStats((prev) => ({
        ...
        averageStakingPeriod:
          (prev.averageStakingPeriod * prev.stakersCount + duration) /
          (prev.stakersCount + (userStake ? 0 : 1)),
      }));
    } catch (error) {
  ...
  const handleUnstake = async () => {
    ...
    try {
      ...
      const amountToReturn =
        userStake.amount * (1 - penalty) + userStake.rewards;
  ...
  const amountToNextTier =
    nextTier && amount ? nextTier.minAmount - parseFloat(amount) : null;
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="h4">{stakingStats.apr}%</Typography>
                  </CardContent>
                </Card>
              </Grid>
            </Grid>
                    ...
                    <Box sx={{ mt: 1 }}>
                      {amountToNextTier && amountToNextTier > 0 ? (
                        <Typography variant="body2" color="text.secondary">
                          Stakez {amountToNextTier.toLocaleString()} $TKN de
                          plus pour atteindre le niveau {nextTier.name}
                        </Typography>
                      ) : (
                        <Typography variant="body2" color="primary">
                          Vous atteindrez le niveau {nextTier.name} avec{" "}
                          {nextTier.discountPercentage}% de réduction
                        </Typography>
                      )}
                    </Box>
                  )}
                ...
                <Box sx={{ mb: 3 }}>
                  <Alert severity="info">
                    <Typography variant="body2">
                      ...
                      {amount && duration
                        ? (
                            ((parseFloat(amount) * stakingStats.apr) / 100) *
                            (duration / 365)
                          ).toFixed(2)
                        : "0"}{" "}
                      $TKN
              ...
              <Box>
                <Grid container spacing={2} sx={{ mb: 3 }}>
                  ...
                  <Grid item xs={6}>
                    ...
                    <Typography variant="h6">{getTimeRemaining()}</Typography>
                  </Grid>
                </Grid>
                ...
                <Box sx={{ mb: 3 }}>
                  <Alert
                    severity={
                      new Date() > userStake.lockedUntil ? "success" : "info"
                    }
                    sx={{ mb: 2 }}
                  >
                ...
                <Button
                  ...
                  variant="contained"
                  color={
                    new Date() > userStake.lockedUntil ? "success" : "primary"
                  }
                  size="large"
                ...
                >
                  {isStaking
                    ? "Traitement..."
                    : "Retirer les tokens et récompenses"}
                </Button>
              </Box>
            )}
          </Paper>
