...
import {
  PaymentError,
  PaymentErrorType,
} from "../../multi-chain/services/payment/types/PaymentError";
...
export function usePaymentFlow() {
    ...
    async (network: PaymentNetwork) => {
      try {
        ...
        setState((prev) => ({
          ...
          status: "error",
          error:
            error instanceof Error ? error : new Error("Failed to set network"),
        }));
      }
    },
  ...
  const validatePayment = useCallback(async () => {
    ...
    await validatePaymentParams(token as Address, amountBigInt, "payment");
