...
const calculateSecurityScore = (config: AntiRugpullConfig): number => {
  ...
  if (config.liquidityLocking.enabled) {
    score +=
      20 *
      (config.liquidityLocking.percentage / 100) *
      Math.min(config.liquidityLocking.duration / 12, 1);
  }
  ...
  if (config.teamVesting.enabled) {
    score +=
      20 *
      (1 - config.teamVesting.initialRelease / 100) *
      Math.min(config.teamVesting.vestingPeriod / 12, 1);
  }
...
const AntiRugpullConfiguratorPage: React.FC = () => {
  ...
  const updateConfigSection = useCallback(
    <T extends keyof AntiRugpullConfig, K extends keyof AntiRugpullConfig[T]>(
      section: T,
      field: K,
  ...
  const activeMechanisms = useMemo(() => {
    ...
    if (config.liquidityLocking.enabled) {
      mechanisms.push(
        `Verrouillage de ${config.liquidityLocking.percentage}% de la liquidité pendant ${config.liquidityLocking.duration} mois`
      );
    }

    if (config.timelock.enabled) {
      mechanisms.push(
        `Timelock de ${config.timelock.delay} heures sur les fonctions critiques`
      );
    }

    if (config.teamVesting.enabled) {
      mechanisms.push(
        `Vesting d'équipe avec ${config.teamVesting.initialRelease}% de libération initiale et ${config.teamVesting.vestingPeriod} mois de vesting`
      );
    }

    if (config.multiSig.enabled) {
      mechanisms.push(
        `Multi-signature avec ${config.multiSig.threshold}/${config.multiSig.owners} signatures requises`
      );
    }

    if (config.transparencyMode.enabled) {
      const transparencyFeatures = [];
      if (config.transparencyMode.publicTeamWallets)
        transparencyFeatures.push("wallets publics");
      if (config.transparencyMode.regularReporting)
        transparencyFeatures.push("rapports réguliers");
      mechanisms.push(
        `Mode transparence${
          transparencyFeatures.length > 0
            ? ` (${transparencyFeatures.join(", ")})`
            : ""
        }`
      );
    }
    ...
    if (config.maxSellLimit.enabled) {
      mechanisms.push(
        `Limite de vente de ${config.maxSellLimit.limitPercentage}% par ${config.maxSellLimit.timeWindow} heures`
      );
    }
    ...
    <>
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Typography variant="subtitle1" color="text.secondary" sx={{ mb: 4 }}>
          Configurez des mécanismes de sécurité avancés pour protéger votre
          token et rassurer votre communauté
        </Typography>
          ...
          <Grid item xs={12} md={8}>
            <Paper sx={{ p: 3, mb: 4 }}>
              ...
              <Typography variant="body2" color="text.secondary" paragraph>
                Configurez les différents mécanismes anti-rugpull pour sécuriser
                votre token.
              </Typography>
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(_, checked) =>
                        updateConfigSection("timelock", "enabled", checked)
                      }
                    />
                  }
