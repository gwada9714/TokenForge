...
const AutoLiquidityManagerPage: React.FC = () => {
    ...
    const handleSettingsChange = (field: keyof LiquiditySettings) => (
        e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
    ) => {
        setLiquiditySettings({
            ...liquiditySettings,
            [field]: field === 'autoLiquidityFee' ? parseFloat(e.target.value) : e.target.value,
        });
    };

    // Mise à jour du formulaire d'ajout de liquidité
    const handleAddLiquidityChange = (field: keyof typeof addLiquidityForm) => (
        e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>
    ) => {
        setAddLiquidityForm({
            ...addLiquidityForm,
            [field]: field === 'tokenAmount' || field === 'lockPeriod' ? e.target.value : e.target.value,
        });
    };
    ...
    const handleAddLiquidity = (e: React.FormEvent) => {
        e.preventDefault();
        alert(`Ajout de liquidité simulé: ${addLiquidityForm.tokenAmount} ${tokenData.symbol} sur ${addLiquidityForm.dex} (${addLiquidityForm.pair})`);
                                ...
                                {liquidityPools.length === 0 ? (
                                    <div className="text-center py-8">
                                        ...
                                        <p className="mt-2 text-gray-600 dark:text-gray-400">
                                            Ajoutez de la liquidité pour permettre les échanges de votre token.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="overflow-x-auto">
                                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            <thead>
                                                <tr>
                                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">DEX / Paire</th>
                                                    ...
                                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Tokens</th>
                                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">APR</th>
                                                    ...
                                                    <tr key={pool.id}>
                                                        ...
                                                        <td className="px-4 py-4 whitespace-nowrap">
                                                            {pool.lockPeriod ? (
                                                                <div>
                                                                    ...
                                                                    <div className="text-sm text-gray-500 dark:text-gray-400">
                                                                        Jusqu'au {formatDate(pool.lockEndDate || '')}
                                                                    </div>
                                                                </div>
                                                            ) : (
                                                                <div className="text-sm text-gray-500 dark:text-gray-400">
                                                        ...
                                                        <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                            <button
                                                                ...
                                                                onClick={() => alert(`Ajouter de la liquidité à ${pool.pair}`)}
                                                            >
                                                                Ajouter
                                                            </button>
                                                            <button
                                                                className={`${pool.lockPeriod ? 'text-gray-400 dark:text-gray-600 cursor-not-allowed' : 'text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300'}`}
                                                                disabled={!!pool.lockPeriod}
                                                                onClick={() => !pool.lockPeriod && alert(`Retirer de la liquidité de ${pool.pair}`)}
                                                            >
                                                                Retirer
                                                            </button>
                                                        </td>
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                                ...
                                <form onSubmit={handleAddLiquidity}>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                        <div>
                                            <label htmlFor="dex" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                        ...
                                        <div>
                                            <label htmlFor="pair" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                                Paire
                                            </label>
                                            <select
                                                id="pair"
                                        ...
                                        <div>
                                            ...
                                            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                                0 = pas de verrouillage. Le verrouillage de liquidité rassure les investisseurs.
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label htmlFor="autoLiquidityFee" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                        ...
                                        />
                                        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                            Pourcentage de chaque transaction qui sera automatiquement ajouté à la liquidité.
                                        </p>
                                    </div>
                                    <div>
                                        <label htmlFor="autoLiquidityThreshold" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                        ...
                                        />
                                        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                            Montant minimum de tokens à accumuler avant d'ajouter à la liquidité.
                                        </p>
                                    </div>
                                    <div>
                                        <label htmlFor="preferredDex" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                            DEX préféré
                                        </label>
                                        <select
                                            id="preferredDex"
                                    ...
                                    <div>
                                        <label htmlFor="preferredPair" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                            Paire préférée
                                        </label>
                                        <select
                                            id="preferredPair"
                            ...
                            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <div className="text-sm text-gray-500 dark:text-gray-400">Liquidité totale</div>
                                        <div className="text-2xl font-bold text-gray-900 dark:text-white">
                                            ${liquidityPools.reduce((sum, pool) => sum + pool.liquidity, 0).toLocaleString()}
                                        </div>
                                    </div>
                                    <div>
                                        <div className="text-sm text-gray-500 dark:text-gray-400">Tokens en liquidité</div>
                                        <div className="text-lg font-medium text-gray-900 dark:text-white">
                                            {liquidityPools.reduce((sum, pool) => sum + parseInt(pool.tokenAmount.replace(/,/g, '')), 0).toLocaleString()} {tokenData.symbol}
                                        </div>
                                        <div className="text-sm text-gray-500 dark:text-gray-400">
                                            ({(liquidityPools.reduce((sum, pool) => sum + parseInt(pool.tokenAmount.replace(/,/g, '')), 0) / parseInt(tokenData.balance.replace(/,/g, '')) * 100).toFixed(2)}% du supply)
                                        </div>
                                    </div>
                                    <div>
                                        <div className="text-sm text-gray-500 dark:text-gray-400">APR moyen</div>
                                        <div className="text-lg font-medium text-green-600 dark:text-green-400">
                                            {(liquidityPools.reduce((sum, pool) => sum + pool.apr, 0) / liquidityPools.length).toFixed(2)}%
                                        </div>
                                    </div>
                                    <div>
                                        <div className="text-sm text-gray-500 dark:text-gray-400">Liquidité verrouillée</div>
                                        <div className="text-lg font-medium text-gray-900 dark:text-white">
                                            ${liquidityPools.filter(pool => pool.lockPeriod).reduce((sum, pool) => sum + pool.liquidity, 0).toLocaleString()}
                                        </div>
                                        <div className="text-sm text-gray-500 dark:text-gray-400">
                                            ({(liquidityPools.filter(pool => pool.lockPeriod).reduce((sum, pool) => sum + pool.liquidity, 0) / liquidityPools.reduce((sum, pool) => sum + pool.liquidity, 0) * 100).toFixed(2)}% de la liquidité totale)
                                        </div>
                                    </div>
                                </div>
                            </div>
