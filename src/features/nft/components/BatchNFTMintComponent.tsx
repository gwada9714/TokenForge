...
export const BatchNFTMintComponent: React.FC = () => {
  ...
  const handleMintCollection = async () => {
    if (!collectionName || nftCount <= 0 || nftCount > 500) {
      setError(
        "Veuillez entrer un nom de collection valide et un nombre de NFTs entre 1 et 500"
      );
      return;
    }
    ...
    try {
      ...
      const nftIds = await createNFTCollection(
        collectionName,
        ownerId,
        nftCount,
        baseMetadata
      ...
      setError(
        (err as Error).message ||
          "Une erreur s'est produite lors de la création de la collection"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Paper sx={{ p: 3, maxWidth: 800, mx: "auto", mt: 4 }}>
      ...
      <Typography variant="body1" paragraph>
        Ce composant utilise les opérations batch de Firestore pour créer une
        collection entière de NFTs en une seule transaction, ce qui optimise les
        performances et garantit l'atomicité.
      </Typography>
      ...
      {mintedNFTIds.length > 0 && (
        <Box sx={{ mt: 4 }}>
          ...
          <Paper
            elevation={1}
            sx={{ p: 2, mt: 1, maxHeight: 150, overflow: "auto" }}
          >
