...
export const TokenForgeAuthContext = createContext<
  TokenForgeAuthContextValue | undefined
>(undefined);
...
export const useTokenForgeAuth = () => {
  ...
  if (!context) {
    const errorMessage =
      "useTokenForgeAuth doit être utilisé à l'intérieur d'un TokenForgeAuthProvider";
    logger.error({
      category: "Auth",
...
export const TokenForgeAuthProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const { address, connect, disconnect, hasInjectedProvider, walletState } =
    useWalletStatus();
  ...
  useEffect(() => {
    if (walletState) {
      ...
      const hasChanged =
        ...
        currentState.isCorrectNetwork !==
          walletConnectionState.isCorrectNetwork;
    ...
    const initAuth = async () => {
      try {
        ...
        logger.info({
          category: "Auth",
          message:
            "Contexte d'authentification TokenForge initialisé avec succès",
        });
      } catch (error) {
        logger.error({
          category: "Auth",
          message:
            "Erreur lors de l'initialisation du contexte d'authentification",
