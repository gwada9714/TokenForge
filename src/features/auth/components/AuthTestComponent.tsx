...
import { Box, Button, TextField, Typography, Alert, CircularProgress, Divider, Card, Stack } from '@mui/material';
...
export const AuthTestComponent: React.FC = () => {
  ...
  const [testResults, setTestResults] = useState<Array<{ label: string; success: boolean; message: string }>>([]);
  const [firebaseTestResults, setFirebaseTestResults] = useState<Record<string, any>>({});
  ...
  const handleDisconnectWallet = async () => {
    try {
      ...
      addTestResult('Déconnexion Wallet', true, 'Wallet déconnecté avec succès');
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
  ...
  const runFirebaseTest = async (testName: string, testFunction: () => Promise<any>) => {
    setIsLoading(true);
    ...
    <Box sx={{ p: 4, maxWidth: 800, mx: 'auto' }}>
      ...
      <Card variant="outlined" sx={{ p: 3, mb: 4 }}>
        ...
        <Box sx={{ mb: 2 }}>
          <Typography variant="body1">
            <strong>Status d'initialisation:</strong> {isInitialized ? '✅ Initialisé' : '⏳ En cours d\'initialisation'}
          </Typography>
          <Typography variant="body1">
            <strong>Status d'authentification:</strong> {isAuthenticated ? '✅ Connecté' : '❌ Déconnecté'}
          </Typography>
          <Typography variant="body1">
          ...
          {wallet && (
            <>
              <Typography variant="body1">
                <strong>Wallet:</strong> {wallet.isConnected ? '✅ Connecté' : '❌ Déconnecté'}
              </Typography>
              <Typography variant="body1">
      ...
      <Box sx={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
        ...
        <Card variant="outlined" sx={{ p: 3, mb: 4, flex: '1 1 400px' }}>
          <Typography variant="h6" gutterBottom>Tests par email</Typography>
          ...
          <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
            <Button variant="contained" onClick={handleTestSignIn}>Connexion</Button>
            <Button variant="contained" color="secondary" onClick={handleTestSignUp}>Inscription</Button>
            <Button variant="outlined" color="error" onClick={handleTestSignOut}>Déconnexion</Button>
          </Box>
        </Card>

        {/* Tests de connexion wallet */}
        <Card variant="outlined" sx={{ p: 3, mb: 4, flex: '1 1 400px' }}>
          <Typography variant="h6" gutterBottom>Tests de wallet</Typography>
          
          <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
            <Button variant="contained" onClick={handleConnectWallet}>Connecter Wallet</Button>
            <Button variant="outlined" color="error" onClick={handleDisconnectWallet}>Déconnecter Wallet</Button>
          </Box>
        </Card>
      </Box>
      ...
      <Card variant="outlined" sx={{ p: 3, mb: 4 }}>
        <Typography variant="h6" gutterBottom>Tests des services Firebase Auth</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Ces tests vérifient directement les services Firebase Auth sous-jacents.
        </Typography>
        ...
        <Stack direction="row" spacing={2} sx={{ mb: 3 }}>
          <Button 
            ...
            color="primary"
            disabled={isLoading}
            onClick={() => runFirebaseTest('initialization', AuthTest.testFirebaseInitialization)}
          >
          ...
          <Button 
            ...
            color="secondary"
            disabled={isLoading}
            onClick={() => runFirebaseTest('anonymousSignIn', AuthTest.testAnonymousSignIn)}
          >
            Connexion anonyme
          </Button>
          <Button 
        ...
        {Object.entries(firebaseTestResults).map(([testName, result]) => (
          <Box key={testName} sx={{ mb: 2, p: 2, bgcolor: 'background.paper', borderRadius: 1 }}>
      ...
      {testResults.length > 0 && (
        <Card variant="outlined" sx={{ p: 3 }}>
          <Typography variant="h6" gutterBottom>Résultats des tests</Typography>
          {testResults.map((result, index) => (
            <Alert key={index} severity={result.success ? 'success' : 'error'} sx={{ mb: 1 }}>
