...
export const AuthButton: React.FC<AuthButtonProps> = ({
    ...
    try {
      ...
      if (session) {
        ...
        if (requiredChainId && chainId !== requiredChainId) {
          ...
          if (!networkSwitched) {
            throw new Error(
              `Veuillez vous connecter au réseau ${SUPPORTED_CHAINS[requiredChainId]}`
            );
          }
        }
        ...
        if (connected) {
          try {
            ...
            console.error(
              "Failed to sign in with wallet after connect:",
              error
            );
            throw new Error(
              "Échec de la connexion à Firebase après connexion du wallet"
            );
          }
        } else {
  ...
  const getButtonText = () => {
    if (isConnectingWallet || isConnectingFirebase)
  ...
  const isLoading =
    isConnectingWallet || isConnectingFirebase || isChangingNetwork;
