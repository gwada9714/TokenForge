...
export const sessionInfoSchema = z.object({
  ...
  lastSync: timestampSchema.optional().describe('Dernier timestamp de synchronisation')
});
...
export const userSchema = z.object({
  ...
  isAdmin: z.boolean().optional().describe('Indique si l\'utilisateur est administrateur'),
  customMetadata: customMetadataSchema.optional().describe('Métadonnées personnalisées')
});
...
export const storedAuthStateSchema = z.object({
  ...
  lastProvider: z.string().nullable().describe('Dernier fournisseur d\'authentification utilisé'),
  trustedDevices: z.array(z.string()).optional().describe('Liste des appareils de confiance'),
  sessionExpiry: timestampSchema.optional().describe('Timestamp d\'expiration de la session'),
...
export const authStateSchema = z.object({
  ...
  }).optional().describe('Dernière erreur survenue')
});
...
export const validatePartialSessionInfo = (data: unknown): Partial<SessionInfo> => {
