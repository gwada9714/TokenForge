...
export const useAuthValidation = () => {
  const validateData = useCallback(
    <T>(validator: (data: unknown) => T, data: unknown, context: string): T => {
      try {
        ...
        if (error instanceof ZodError) {
          ...
          throw createAuthError(
            ...
            `Données invalides pour ${context}: ${error.errors
              .map((e) => e.message)
              .join(", ")}`,
  ...
  const validateCurrentState = useCallback(
    (data: unknown): AuthState => {
      return validateData(validateAuthState, data, "current state");
    },
    [validateData]
  );

  const validatePartialUserData = useCallback(
    (data: unknown): Partial<User> => {
      return validateData(validatePartialUser, data, "partial user");
    },
    [validateData]
  );

  const validatePartialSessionData = useCallback(
    (data: unknown): Partial<SessionInfo> => {
