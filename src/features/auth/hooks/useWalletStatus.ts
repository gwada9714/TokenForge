import { WalletConnectionState, WalletConnectionStatus } from '../../../types/authTypes';
import { useAccount, useChainId, useSwitchChain, useConnect, useDisconnect, useWalletClient } from 'wagmi';
  ...
  const checkWalletAvailability = async (): Promise<boolean> => {
    try {
      ...
      if (hasEthereum) {
        ...
        const hasProvider = isMetaMask || isTrust || isCoinbase || isWalletConnect;
        ...
        const validConnectors = connectors.filter(connector => 
          connector.ready || connector.id === 'walletConnect' // WalletConnect est toujours "ready"
        );
  ...
  const connect = async (): Promise<void> => {
    try {
      if (!hasInjectedProvider) {
        ...
        throw new Error('Aucun wallet détecté. Veuillez installer MetaMask ou un wallet compatible.');
      }
      
      // Trouver le premier connecteur disponible
      const availableConnector = connectors.find(c => c.ready || c.id === 'walletConnect');
