...
export const useFirebaseAuth = () => {
  ...
  const authServiceRef = useRef<Awaited<ReturnType<typeof getFirebaseAuthService>> | null>(null);
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        unsubscribeRef.current = authServiceRef.current.onAuthStateChange((user) => {
          setState({
            user,
            isLoading: false,
            error: null
        ...
        logger.log(LogLevel.ERROR, 'Erreur lors de l\'initialisation de l\'authentification:', error);
        setState({
          user: null,
          isLoading: false,
  ...
  const signIn = useCallback(async (email: string, password: string) => {
    if (!authServiceRef.current) throw new Error('Service d\'authentification non initialisé');
    
    try {
      setState(prev => ({ ...prev, isLoading: true, error: null }));
      const user = await authServiceRef.current.signInWithEmail(email, password);
