...
vi.mock("../retryService", () => ({
  retryService: {
    withRetry: vi
      .fn()
      .mockImplementation(async (fn, config, retryableErrors) => {
    ...
    withTimeout: vi
      .fn()
      .mockImplementation(async (promise, _timeout, operation) => {
...
describe("TokenService", () => {
  ...
  const mockExpirationTime = new Date(
    Date.now() + 60 * 60 * 1000
  ...
  describe("initialize", () => {
    ...
    it("should handle session expired error", async () => {
      ...
      await expect(tokenService.initialize(mockUser)).rejects.toBe(
        sessionError
      );
