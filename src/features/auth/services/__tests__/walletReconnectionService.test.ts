...
describe("WalletReconnectionService", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(networkRetryService.retryWithTimeout).mockImplementation(
      async (fn) => {
        try {
          await fn();
  ...
  describe("attemptReconnection", () => {
    it("devrait se reconnecter avec succès", async () => {
      ...
      expect(tabSyncService.broadcast).toHaveBeenCalledWith(
  ...
  describe("setupNetworkChangeListener", () => {
    it("devrait gérer les changements de réseau", async () => {
      ...
      if (mockEthereumProvider._chainChangedCallback) {
        await mockEthereumProvider._chainChangedCallback(
          `0x${newChainId.toString(16)}`
        );

        // Attendre que les callbacks soient appelés
        await vi.waitFor(async () => {
          expect(mockCallbacks.onNetworkChange).toHaveBeenCalledWith(
            newChainId
          );
