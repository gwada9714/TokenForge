...
describe('SessionService', () => {
  ...
  let syncSubscriber: ((message: { type: string; timestamp: number; tabId: string; payload?: any }) => void) | null = null;
  ...
  describe('initSession', () => {
    ...
    it('devrait gérer les erreurs d\'initialisation', async () => {
      ...
      await expect(sessionService.initSession()).rejects.toThrow('Failed to initialize session');
    });
  ...
  describe('error handling', () => {
    it('devrait gérer les erreurs de rafraîchissement de session', async () => {
      vi.mocked(mockedAuthPersistence.load).mockRejectedValueOnce(new Error('Load failed'));

      await sessionService.refreshSession();
      expect(mockedNotificationService.error).toHaveBeenCalledWith('Error refreshing session');
    });

    it('devrait gérer les erreurs de synchronisation', async () => {
      vi.mocked(mockedAuthPersistence.save).mockRejectedValueOnce(new Error('Save failed'));
