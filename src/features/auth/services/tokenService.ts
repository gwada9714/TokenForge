...
class TokenService {
  ...
  async initialize(user: User | null): Promise<void> {
    logService.info(LOG_CATEGORY, "Initializing token service", {
      userId: user?.uid,
    });
  ...
  private async refreshToken(): Promise<void> {
    try {
      ...
      logService.debug(LOG_CATEGORY, "Starting token refresh", {
  ...
  async getToken(): Promise<string> {
    if (!this.tokenInfo) {
      ...
      logService.error(
        LOG_CATEGORY,
        "Token request failed - no token available",
        error
      );
      throw error;
    }
  ...
  isTokenExpired(): boolean {
    const isExpired =
      !this.tokenInfo || this.tokenInfo.expirationTime <= Date.now();
    if (isExpired) {
