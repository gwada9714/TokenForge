...
export class TokenPriceService {
    ...
    try {
      ...
      Object.entries(COINGECKO_IDS).forEach(([symbol, id]) => {
        ...
        if (price) {
          ...
          const finalPrice = ["USDT", "USDC", "DAI", "BUSD"].includes(symbol)
            ? 1
            : price;
  ...
  private isTokenSupportedOnNetwork(
    symbol: string,
  ...
  private listeners: Set<(prices: Map<PaymentNetwork, TokenPriceMap>) => void> =
    new Set();

  public onPriceUpdate(
    callback: (prices: Map<PaymentNetwork, TokenPriceMap>) => void
  ): () => void {
