...
export class BinancePaymentService extends AbstractChainService {
  ...
  async createPaymentSession(params: {
    ...
    if (network.chainId !== 56n) { // BSC Mainnet
      throw this.createPaymentError('Réseau BSC non disponible', 'NETWORK_ERROR');
    }
  ...
  private async checkBalanceAndAllowance(session: PaymentSession): Promise<void> {
    if (session.token.address === '0x0000000000000000000000000000000000000000') {
      ...
      if (balance < amount) {
        throw this.createPaymentError('Solde BNB insuffisant', 'INSUFFICIENT_BALANCE');
      }
    } else {
