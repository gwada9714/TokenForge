...
describe("SolanaService", () => {
  ...
  const testAddress =
    "0xGsbwXfJraMomNxBcpR5WTNdtx1BgwYhQdThwcWvJuf8p" as Address;
  ...
  describe("getBalance", () => {
    ...
    it("should throw error when connection fails", async () => {
      vi.mocked(mockConnection.getBalance).mockRejectedValueOnce(
  ...
  describe("createToken", () => {
    ...
    it("should throw error when token creation fails", async () => {
      vi.mocked(createMint).mockRejectedValueOnce(
  ...
  describe("getTokenInfo", () => {
    ...
    it("should throw error when getting token info fails", async () => {
      vi.mocked(getMint).mockRejectedValueOnce(
        new Error("Failed to get mint info")
      );

      await expect(service.getTokenInfo(testAddress)).rejects.toThrow(
