...
import { NetworkTokensConfig, COMMON_TOKEN_ADDRESSES } from '../types/TokenConfig';
...
export const getNetworkConfig = (network: PaymentNetwork): NetworkTokensConfig => {
...
export const getTokenConfig = (network: PaymentNetwork, tokenAddress: Address | PublicKey) => {
  const networkConfig = getNetworkConfig(network);
  const token = networkConfig.tokens.find(t => t.address === tokenAddress);
  if (!token) {
    throw new Error(`Token ${tokenAddress.toString()} not supported on network ${network}`);
  }
  return token;
};
