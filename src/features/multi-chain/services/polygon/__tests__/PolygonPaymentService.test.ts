...
describe("PolygonPaymentService", () => {
  ...
  describe("getInstance", () => {
    ...
    it("should throw error when getting instance without initialization", () => {
      PolygonPaymentService.resetInstance();
      expect(() => PolygonPaymentService.getInstance()).toThrow(
        "PolygonPaymentService not initialized"
      );
    });
  ...
  describe("payWithToken", () => {
    ...
    it("should use fallback values when gas estimation fails", async () => {
      vi.spyOn(mockPublicClient, "getBlock").mockRejectedValue(
        new Error("Failed to get block")
      );
      vi.spyOn(
        mockPublicClient,
        "estimateMaxPriorityFeePerGas"
      ).mockRejectedValue(new Error("Failed to estimate priority fee"));
      vi.spyOn(mockPublicClient, "estimateContractGas").mockRejectedValue(
