...
describe('useSubscription', () => {
  ...
  it('should handle successful subscription', async () => {
    ...
    await act(async () => {
      const success = await result.current.subscribe(SubscriptionTier.FORGERON, '0xtxhash');
      expect(success).toBe(true);
    });

    expect(result.current.currentSubscription?.tier).toBe(SubscriptionTier.FORGERON);
    expect(result.current.error).toBeNull();
  });
