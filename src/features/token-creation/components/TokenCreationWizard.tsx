...
export const TokenCreationWizard: React.FC = () => {
  ...
  const [selectedNetwork, setSelectedNetwork] =
    useState<BlockchainNetwork>("bsc");
  ...
  useEffect(() => {
    if (activeStep === 2 && !isConnected) {
      setValidationErrors([
        "Veuillez connecter votre wallet pour déployer un token",
      ]);
    } else {
      setValidationErrors([]);
    }
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      const validation = await deploymentService.validateTokenConfig(
        tokenConfig
      );
                ...
                {deploymentResult.success ? (
                  <Alert severity="success">
                    <Typography variant="subtitle1">
                      Token déployé avec succès!
                    </Typography>
                    <Typography variant="body2">
                      Adresse: {deploymentResult.tokenAddress}
                    </Typography>
                    {deploymentResult.transactionHash && (
                      <Typography variant="body2">
                        Transaction:{" "}
                        {deploymentResult.transactionHash.substring(0, 10)}...
                        {deploymentResult.transactionHash.substring(
                          deploymentResult.transactionHash.length - 8
                        )}
                      </Typography>
                    )}
                  </Alert>
