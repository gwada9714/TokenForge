...
export const styledTheme: DefaultTheme = {
  ...
  typography: {
    fontFamily: {
      ...
      code: THEME_CONFIG.typography.fontFamily.code || '"JetBrains Mono", monospace'
    },
    fontWeight: {
  ...
  breakpoints: {
    ...
    between: (start: string, end: string): string => {
      ...
      return `@media (min-width: ${breakpointValues[startKey]}px) and (max-width: ${breakpointValues[endKey] - 0.05}px)`;
    }
  }
};
