...
export class TemplateService {
  ...
  public searchTemplates(options: TemplateSearchOptions = {}): Template[] {
    ...
    if (options.type) {
      templates = templates.filter(
        (template) => template.type === options.type
      );
    }
    ...
    if (options.license) {
      templates = templates.filter(
        (template) => template.license === options.license
      );
    }
    ...
    if (options.minPrice !== undefined) {
      templates = templates.filter(
        (template) => template.price >= options.minPrice!
      );
    }

    if (options.maxPrice !== undefined) {
      templates = templates.filter(
        (template) => template.price <= options.maxPrice!
      );
    }
  ...
  public addTemplate(
    template: Omit<
      Template,
      "id" | "createdAt" | "updatedAt" | "rating" | "ratingCount" | "downloads"
    >
  ): string {
    ...
    logger.info(
      "TemplateService",
      `Nouveau template ajouté: ${template.name}`,
      {
        templateId,
  ...
  public purchaseLicense(
    ...
    const licenseId = `license_${Date.now()}_${Math.random()
      .toString(36)
      .substring(2, 9)}`;
    ...
    logger.info(
      "TemplateService",
      `Licence achetée pour le template: ${template.name}`,
      {
        licenseId,
        templateId,
        userId,
  ...
  public hasValidLicense(templateId: string, userId: string): boolean {
    ...
    const templateLicenses = userLicenses.filter(
      (license) => license.templateId === templateId && license.isActive
    );
  ...
  public useLicense(templateId: string, userId: string): boolean {
    ...
    logger.info(
      "TemplateService",
      `Licence utilisée pour le template: ${templateId}`,
      {
  ...
  private loadSampleTemplates(): void {
    const sampleTemplates: Template[] = [
      {
        ...
        description:
          "Un token ERC-20 standard avec les fonctionnalités de base.",
      ...
      },
      {
        ...
        description:
          "Un token optimisé pour les applications DeFi avec génération de rendement.",
      ...
      },
      {
        ...
        name: "Gaming Token",
        description:
          "Un token conçu pour les jeux blockchain avec fonctionnalités de récompense.",
      ...
      },
      {
        ...
        description:
          "Un token de gouvernance pour les organisations autonomes décentralisées.",
      ...
      },
      {
        ...
        description:
          "Un token pour les créateurs de contenu et les communautés sociales.",
    ...
    logger.info(
      "TemplateService",
      `${sampleTemplates.length} templates d'exemple chargés`
    );
  }
}
