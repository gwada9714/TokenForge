...
export class TokensController {
  ...
  constructor(
    factoryContract: ReturnType<typeof getContract>,
  ...
  public getUserTokens = async (
    ...
    try {
      ...
      const tokenAddresses = await this.tokenForgeFactory.getUserTokens(
        address
      );
      ...
      if (owner) {
        ...
        tokens = await Promise.all(
          paginatedAddresses.map(async (addr: string) => {
            ...
            const [name, tokenSymbol, decimals, totalSupply, tokenOwner] =
              await Promise.all([
                token.name(),
                token.symbol(),
                token.decimals(),
