...
describe('AuditLogList', () => {
  ...
  describe('Affichage des Logs', () => {
    it('devrait afficher correctement les logs', () => {
      ...
      mockLogs.forEach(log => {
        ...
        expect(screen.getByText(new Date(log.timestamp).toLocaleString())).toBeInTheDocument();
      });
    });
  ...
  describe('Tri et Filtrage', () => {
    ...
    it('devrait trier les logs par niveau de gravité', async () => {
      ...
      const levels = screen.getAllByTestId('log-level')
        .map(el => el.textContent);

      const severityOrder = { error: 0, warning: 1, info: 2 };
      const isSorted = levels.every((level, i) => 
        i === 0 || severityOrder[level as keyof typeof severityOrder] >= 
  ...
  describe('Actions sur les Logs', () => {
    ...
    it('devrait appeler onViewDetails avec le log correct', () => {
      ...
      const firstLogContainer = screen.getByText(mockLogs[0].message).closest('li');
      if (!firstLogContainer) throw new Error('Log container not found');
      
      const viewButton = within(firstLogContainer).getByRole('button', { name: /voir les détails/i });
  ...
  describe('Accessibilité', () => {
    it('devrait avoir les attributs ARIA appropriés', () => {
      ...
      expect(screen.getByRole('list')).toHaveAttribute('aria-label', 'audit logs');
      
      const deleteButtons = screen.getAllByTestId(/delete-button-/);
      deleteButtons.forEach((button, index) => {
        expect(button).toHaveAttribute('aria-label', `delete log ${mockLogs[index].action}`);
      });
    });
    ...
    it('devrait être navigable au clavier', async () => {
      ...
      const firstViewButton = screen.getAllByRole('button', { name: /voir les détails/i })[0];
  ...
  describe('Performance', () => {
    ...
    it('devrait optimiser les re-rendus', async () => {
      ...
      vi.spyOn(console, 'log').mockImplementation(() => { renderCount.value++; });
