...
describe("Firebase Auth Service", () => {
  ...
  describe("signInWithEmail", () => {
    ...
    it("should throw AuthError on sign in failure", async () => {
      ...
      await expect(
        firebaseAuth.signInWithEmail("test@example.com", "wrong")
      ).rejects.toMatchObject({ code: AuthErrorCode.SIGN_IN_ERROR });
    });
  });
  ...
  describe("updateUserProfile", () => {
    ...
    it("should throw error when no user is signed in", async () => {
      ...
      await expect(
        firebaseAuth.updateUserProfile("New Name")
      ).rejects.toMatchObject({ code: AuthErrorCode.USER_NOT_FOUND });
    });
  });
