...
describe('Auth Errors', () => {
  describe('AuthError', () => {
    it('devrait créer AuthError avec code et message', () => {
      const error = new AuthError(AuthErrorCode.INVALID_CREDENTIALS, 'Erreur d\'authentification');
    ...
    it('devrait stocker l\'erreur originale', () => {
      ...
      const error = new AuthError(AuthErrorCode.INTERNAL_ERROR, 'Erreur interne', originalError);
  ...
  describe('createAuthError', () => {
    it('devrait créer une instance de AuthError', () => {
      const error = createAuthError(AuthErrorCode.INVALID_EMAIL, 'Email invalide');
    ...
    it('devrait inclure l\'erreur originale si fournie', () => {
      const originalError = new Error('Erreur originale');
      const error = createAuthError(AuthErrorCode.NETWORK_ERROR, 'Erreur réseau', originalError);
      expect(error.originalError).toBe(originalError);
    });
  });
