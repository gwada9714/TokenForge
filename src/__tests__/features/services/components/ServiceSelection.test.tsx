...
const MockServiceSelection: React.FC<{ onServiceSelected: (type: ServiceType) => void }> = ({ onServiceSelected }) => (
  <div>
    {MOCK_SERVICES.map(service => (
      <div key={service.id} onClick={() => onServiceSelected(service.id)}>
        <h3>{service.name}</h3>
        <p>{service.description}</p>
        <p>{service.price.baseFee} {service.price.currency}</p>
        {service.features.map(feature => (
          <div key={feature.id}>
            <span data-testid={`feature-${feature.id}`} title={feature.description}>
              {feature.name}
            </span>
          </div>
        ))}
