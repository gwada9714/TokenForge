...
import { ServiceType, LaunchpadConfig as ILaunchpadConfig } from '@/features/services/types/services';
...
describe('LaunchpadConfig Component', () => {
  ...
  it('affiche les erreurs de validation', async () => {
    ...
    expect(screen.getByText(/La supply totale est requise/i)).toBeInTheDocument();
  });
  ...
  it('valide le format du symbole', async () => {
    ...
    const tokenSymbol = screen.getByLabelText(/Symbole/i);
    await fireEvent.change(tokenSymbol, { target: { value: 'INVALID-SYMBOL' } });
    await fireEvent.blur(tokenSymbol);

    expect(screen.getByText(/Le symbole doit contenir uniquement des lettres majuscules/i)).toBeInTheDocument();
  });
