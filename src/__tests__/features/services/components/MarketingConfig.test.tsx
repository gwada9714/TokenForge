...
describe("MarketingConfig Component", () => {
  ...
  it("gère la soumission du formulaire correctement", async () => {
    ...
    await fireEvent.change(projectDescription, {
      target: { value: "Description détaillée du projet marketing" },
    });
    await fireEvent.change(twitter, { target: { value: "@monprojet" } });
    await fireEvent.change(telegram, { target: { value: "t.me/monprojet" } });
    await fireEvent.change(discord, {
      target: { value: "discord.gg/monprojet" },
    });
    await fireEvent.change(medium, { target: { value: "@monprojet" } });
    await fireEvent.change(budget, { target: { value: "10" } });
    await fireEvent.change(objectives, {
      target: { value: "Augmenter la visibilité" },
    });
    await fireEvent.change(prType, { target: { value: "Lancement" } });
    await fireEvent.change(keyPoints, {
      target: { value: "Points clés du projet" },
    });
  ...
  it("affiche les erreurs de validation", async () => {
    ...
    expect(
      screen.getByText(/Le nom du projet est requis/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/La description du projet est requise/i)
    ).toBeInTheDocument();
    expect(screen.getByText(/Le budget est requis/i)).toBeInTheDocument();
    expect(
      screen.getByText(/Le type de communiqué est requis/i)
    ).toBeInTheDocument();
  });
  ...
  it("valide la longueur minimale de la description", async () => {
    ...
    expect(
      screen.getByText(/La description doit faire au moins 100 caractères/i)
    ).toBeInTheDocument();
  });
  ...
  it("valide que le budget est positif", async () => {
    ...
    expect(
      screen.getByText(/Le budget doit être positif/i)
    ).toBeInTheDocument();
  });
