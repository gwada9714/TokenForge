...
import { ServiceType, StakingConfig as IStakingConfig } from '@/features/services/types/services';
...
describe('StakingConfig Component', () => {
  ...
  it('affiche les erreurs de validation', async () => {
    ...
    expect(screen.getByText(/La période de staking est requise/i)).toBeInTheDocument();
    expect(screen.getByText(/Le token de récompense est requis/i)).toBeInTheDocument();
  });
  ...
  it('valide les valeurs numériques', async () => {
    ...
    expect(screen.getByText(/L'APR doit être supérieur à 0/i)).toBeInTheDocument();
  });
