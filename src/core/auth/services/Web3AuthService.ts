...
export class Web3AuthService extends AuthServiceBase {
  ...
  public async login(credentials: Web3Credentials): Promise<AuthResponse> {
    try {
      if (!this.provider) {
        ...
        if (!this.provider) {
          return {
            success: false,
            error: new Error(
              "Provider Web3 non disponible. Veuillez installer MetaMask ou un wallet compatible."
            ),
          };
        }
      }
      ...
      if (!credentials.signature) {
        const { signature, message } = await this.signMessage(
          credentials.address
        );
  ...
  public async signup(): Promise<AuthResponse> {
    return {
      success: false,
      error: new Error(
        "La méthode signup n'est pas applicable pour l'authentification Web3"
      ),
    };
  }
  ...
  public async resetPassword(): Promise<boolean> {
    throw new Error(
      "La méthode resetPassword n'est pas applicable pour l'authentification Web3"
    );
  }
  ...
  public async updateUserProfile(): Promise<any> {
    throw new Error(
      "La méthode updateUserProfile n'est pas applicable pour l'authentification Web3"
    );
  }
  ...
  private mapToUser(credentials: Web3Credentials): any {
    return {
      ...
      chainId: credentials.chainId,
      isWeb3: true,
      displayName: `${credentials.address.substring(
        0,
        6
      )}...${credentials.address.substring(credentials.address.length - 4)}`,
      photoURL: null,
    };
  }
   ...
   */
  protected async setupTokenRefresh(
