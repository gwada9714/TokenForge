...
export class EnhancedAuthService {
  ...
  private async verifyTokenIntegrity(user: User): Promise<void> {
    try {
      ...
      logger.error(
        "EnhancedAuth",
        "Échec de la vérification d'intégrité du token",
        error
      );
  ...
  private async setupTokenRefresh(user: User): Promise<void> {
    try {
      ...
      try {
        ...
        logger.info("EnhancedAuth", "Token initial obtenu avec succès", {
          tokenLength: token.length,
        });
      } catch (error) {
        logger.error(
          "EnhancedAuth",
          "Échec de l'obtention du token initial",
          error
        );
      ...
      const intervalId = window.setInterval(async () => {
        try {
          ...
          const token = await user.getIdToken(true);
          logger.info("EnhancedAuth", "Token rafraîchi avec succès", {
            tokenLength: token.length,
          });
      ...
      logger.info(
        "EnhancedAuth",
        "Rafraîchissement de token configuré avec succès",
        { userId: user.uid }
      );
    } catch (error) {
      logger.error(
        ...
        return {
          ...
          error: {
            ...
            message: `Trop de tentatives de connexion. Veuillez réessayer après ${
              this.LOGIN_ATTEMPT_RESET_TIME / 60000
            } minutes.`,
          },
        };
      }
  ...
  async logout(): Promise<void> {
    try {
      ...
      if (this.auth.currentUser) {
        ...
        logger.info(
          "EnhancedAuth",
          "Déconnexion ignorée - aucun utilisateur connecté"
        );
      }
    } catch (error) {
