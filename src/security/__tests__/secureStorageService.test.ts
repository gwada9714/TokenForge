...
describe("Secure Storage Service", () => {
  ...
  describe("setItem", () => {
    ...
    it("handles encryption errors", async () => {
      ...
      await expect(storage.setItem("test-key", { foo: "bar" })).rejects.toThrow(
        TokenForgeError
      );
    });

    it("validates input data", async () => {
      await expect(storage.setItem("", { foo: "bar" })).rejects.toThrow(
        TokenForgeError
      );

      await expect(
        storage.setItem("test-key", undefined as any)
  ...
  describe("key rotation", () => {
    it("rotates encryption key", async () => {
      ...
      vi.mocked(mockCrypto.subtle.generateKey).mockResolvedValueOnce(
        mockNewKey
      );
      vi.mocked(mockCrypto.subtle.exportKey).mockResolvedValue(
