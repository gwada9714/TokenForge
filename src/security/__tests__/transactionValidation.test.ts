...
describe("Transaction Validation Service", () => {
  ...
  describe("validateTransaction", () => {
    it("validates a valid transaction", async () => {
      const result = await transactionValidationService.validateTransaction(
        mockTransaction
      );
    ...
    it("detects insufficient balance", async () => {
      // Mock low balance
      vi.mocked(createPublicClient().getBalance).mockResolvedValueOnce(
    ...
    it("validates gas estimation", async () => {
      ...
      vi.mocked(createPublicClient().estimateGas).mockResolvedValueOnce(
        BigInt("500000")
      );
  ...
  describe("validateContractInteraction", () => {
    it("validates simple transfer", async () => {
      const result =
        await transactionValidationService.validateContractInteraction(
          mockTransaction.to,
    ...
    it("validates contract code presence", async () => {
      ...
      vi.mocked(createPublicClient()).getCode = vi
        .fn()
  ...
  describe("validateNonce", () => {
    it("validates correct nonce", async () => {
      // Mock current nonce
      vi.mocked(createPublicClient()).getTransactionCount = vi
        .fn()
