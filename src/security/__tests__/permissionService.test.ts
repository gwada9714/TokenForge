...
describe('Permission Service', () => {
  ...
  describe('grantPermission', () => {
    ...
    it('throws error for invalid permission', async () => {
      ...
      await expect(permissionService.grantPermission(uid, permission)).rejects.toThrow(
        TokenForgeError
      );
    });
  });
  ...
  describe('validateRoleHierarchy', () => {
    it('validates admin can manage users', async () => {
      ...
      const canManage = await permissionService.validateRoleHierarchy(adminUid, targetUid);
    ...
    it('prevents user from managing admin', async () => {
      ...
      const canManage = await permissionService.validateRoleHierarchy(userUid, adminUid);
  ...
  describe('updateRolePermissions', () => {
    it('updates role permissions successfully', async () => {
      const role: Role = 'user';
      const newPermissions: Permission[] = ['create_token', 'view_contracts', 'manage_own_tokens'];
      ...
      vi.mocked(getAuth().listUsers).mockResolvedValueOnce({
        users: [
          {
            uid: 'user1',
            customClaims: { role: 'user', permissions: ['create_token', 'view_contracts'] }
          },
          {
            uid: 'user2',
            customClaims: { role: 'user', permissions: ['create_token', 'view_contracts'] }
    ...
    it('validates required permissions', async () => {
      ...
      await expect(permissionService.updateRolePermissions(role, invalidPermissions))
