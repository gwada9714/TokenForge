...
describe('Session Service', () => {
  ...
  describe('refreshSession', () => {
    ...
    it('fails to refresh expired session', async () => {
      ...
      await expect(sessionService.refreshSession('expired-token')).rejects.toThrow(
        TokenForgeError
      );
    });
    ...
    it('handles refresh errors', async () => {
      ...
      await expect(sessionService.refreshSession('valid-token')).rejects.toThrow(
        TokenForgeError
      );
    });
  });
  ...
  describe('invalidateSession', () => {
    ...
    it('handles already invalidated session', async () => {
      ...
      await expect(sessionService.invalidateSession('invalid-token')).resolves.not.toThrow();
    });
  });
  ...
  describe('validateMultiTabSession', () => {
    it('validates session across tabs', async () => {
      ...
      const isValid = await sessionService.validateMultiTabSession('valid-token');
      expect(isValid).toBe(true);
    });
  ...
  describe('cleanupExpiredSessions', () => {
    it('removes expired sessions', async () => {
      ...
      vi.mocked(mockStorage.getItem)
        .mockImplementation((key) => mockSessions[key]);
