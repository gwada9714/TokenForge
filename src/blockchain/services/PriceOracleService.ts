...
export class PriceOracleService {
  ...
  async getCryptoPrice(
    ...
    try {
      ...
      if (
        ...
        !response.data[coinId][fiatCurrency.toLowerCase()]
      ) {
        throw new Error(
          `Invalid response from CoinGecko for ${cryptoSymbol}/${fiatCurrency}`
        );
      }
  ...
  private getCoingeckoId(symbol: string): string {
    ...
    if (!id) {
      console.warn(
        `No CoinGecko ID mapping for symbol ${symbol}, using symbol as ID`
      );
