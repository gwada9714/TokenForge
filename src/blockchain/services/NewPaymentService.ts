...
import { getMinConfirmations, getEstimatedTransactionTime, getChainId } from './PaymentServiceUtils';
...
export class PaymentService implements IPaymentService {
  ...
  // Utilisation du préfixe _ pour indiquer que ces variables sont stockées mais pas utilisées directement
  // Ces variables pourraient être utilisées dans une implémentation future
  private readonly _walletProvider: any;
  private readonly _paymentService: any; // Changé de IPaymentService à any pour éviter les erreurs circulaires
  ...
  constructor(chainName: string, walletProvider?: any) {
    ...
    this._paymentService = createPaymentService(chainName, walletProvider);
  ...
  async initPaymentSession(params: PaymentInitParams): Promise<PaymentSession> {
    try {
      ...
      const currency = supportedCryptos.find(c => c.symbol === params.currency);
      
      if (!currency) {
        throw new Error(`Cryptocurrency ${params.currency} not supported on ${this.chainName}`);
      }
      ...
      const session: PaymentSession = {
        ...
        minConfirmations: getMinConfirmations(this.chainName)
      };
      
      return session;
    } catch (error) {
      console.error(`Error initializing payment session on ${this.chainName}:`, error);
      throw new Error(`Failed to initialize payment session: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   */
  async checkPaymentStatus(paymentSessionId: string): Promise<PaymentStatus> {
    try {
      ...
      console.error(`Error checking payment status on ${this.chainName}:`, error);
   ...
   */
  async confirmPayment(paymentSessionId: string, transactionHash: string): Promise<boolean> {
    try {
  ...
  async getSupportedCryptocurrencies(): Promise<CryptocurrencyInfo[]> {
    try {
      ...
      console.error(`Error getting supported cryptocurrencies for ${this.chainName}:`, error);
      return [];
    }
  }
  ...
  async estimateTransactionFees(amount: number, currencyAddress: string | null): Promise<FeeEstimate> {
    try {
      ...
      if (this.chainName.toLowerCase() === 'solana') {
        ...
        baseFeeAmount = 0.001;
      } else if (this.chainName.toLowerCase() === 'binance' || this.chainName.toLowerCase() === 'bsc') {
      ...
      const nativePriceEUR = await this.priceOracle.getCryptoPrice(nativeSymbol, 'EUR');
      const baseFeeEUR = baseFeeAmount * nativePriceEUR;
      const maxFeeEUR = maxFeeAmount * nativePriceEUR;
      
      // Formater les montants
      const baseFeeCrypto = this.formatCryptoAmount(baseFeeAmount, nativeSymbol, nativeDecimals);
      const maxFeeCrypto = this.formatCryptoAmount(maxFeeAmount, nativeSymbol, nativeDecimals);
      ...
      return {
        ...
        estimatedTimeSeconds: getEstimatedTransactionTime(this.chainName)
      };
    } catch (error) {
      console.error(`Error estimating transaction fees on ${this.chainName}:`, error);
   ...
   */
  async convertEURtoCrypto(amountInEUR: number, currencySymbol: string): Promise<CryptoAmount> {
    try {
      // Récupérer les infos de la crypto
      const supportedCryptos = await this.getSupportedCryptocurrencies();
      const currency = supportedCryptos.find(c => c.symbol === currencySymbol);
      
      if (!currency) {
        throw new Error(`Currency ${currencySymbol} not supported on ${this.chainName}`);
      }
      ...
      if (currency.isStablecoin) {
        ...
        cryptoAmount = amountInEUR * eurUsdRate;
        ...
        cryptoAmount = amountInEUR / cryptoPriceEUR;
      ...
      // Formater le montant
      return this.formatCryptoAmount(cryptoAmount, currencySymbol, currency.decimals, amountInEUR);
    } catch (error) {
      console.error(`Error converting EUR to crypto on ${this.chainName}:`, error);
      throw new Error(`Failed to convert EUR to ${currencySymbol}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async createPaymentSession(amount: bigint, currency: string): Promise<string> {
    try {
      ...
      const amountNumber = Number(amount) / Math.pow(10, this.getDecimals(currency));
      ...
      console.error(`Error creating payment session on ${this.chainName}:`, error);
      throw new Error(`Failed to create payment session: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async getPaymentStatus(sessionId: string): Promise<LegacyPaymentStatus> {
    try {
      ...
      console.error(`Error getting payment status on ${this.chainName}:`, error);
      return {
        status: 'failed',
        details: {
  ...
  async generateReceipt(
    ...
    const isValid = await this.verifyPayment(txHash);
    
    return {
      sessionId,
      transactionHash: txHash,
      amount: this.formatAmount(paymentAmount, currency),
