...
export class SolanaMonitoringService {
  ...
  async startMonitoring() {
    ...
    try {
      ...
      // Démarrer le polling
      this.pollingInterval = setInterval(() => this.checkNewTransactions(), 10000); // Vérifier toutes les 10 secondes
    } catch (error) {
  ...
  private async checkNewTransactions() {
    try {
      ...
      const newSignatures = this.lastSignature 
        ? signatures.filter(sig => sig.signature !== this.lastSignature).reverse()
        : signatures.reverse();
  ...
  private async processTransaction(signature: string) {
    try {
      ...
      console.log(`Simulating Solana transfer detection for signature: ${signature}`);
   ...
   */
  private async processIncomingPayment(from: string, amount: number, txHash: string) {
    try {
      ...
      const confirmed = await this.paymentService.confirmPayment(sessionId, txHash);
