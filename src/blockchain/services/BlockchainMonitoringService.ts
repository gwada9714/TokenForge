...
export class BlockchainMonitoringService {
    ...
    try {
      ...
      for (const networkName of supportedNetworks) {
        ...
        try {
          ...
          console.error(
            `Error initializing provider for ${networkName}:`,
            error
          );
          continue;
        }
        ...
        const supportedCryptos =
          await paymentService.getSupportedCryptocurrencies();
      ...
      console.log(
        `Initialized ${
          Object.keys(this.networks).length
        } blockchain networks for monitoring`
      );
    } catch (error) {
    ...
    try {
      ...
      provider.on(filter, async (log: any) => {
        try {
          ...
          console.error(
            `Error processing ${network} token transfer event:`,
            error
          );
        }
      });
      ...
      console.error(
        `Error setting up ${network} token monitoring for ${tokenAddress}:`,
        error
      );
    }
  }
  ...
  private async processIncomingNativeTransaction(
    ...
    try {
      ...
      console.log(
        `Searching for pending native payment sessions for ${from} on ${network}`
      );
  ...
  private async processIncomingTokenTransaction(
    ...
    try {
      ...
      // Pour la démo, on simule une recherche de session
      console.log(
        `Searching for pending token payment sessions for ${from} on ${network} with token ${tokenAddress}`
      );
