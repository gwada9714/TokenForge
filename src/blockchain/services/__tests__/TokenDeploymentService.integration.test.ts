...
import { TokenDeploymentService, TokenDeploymentOptions } from '../TokenDeploymentService';
...
describe('TokenDeploymentService - Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    tokenDeploymentService.registerBlockchainService('ethereum', mockBlockchainService);
  ...
  it('devrait déployer un token standard avec succès', async () => {
    ...
    const executeDeploymentSpy = vi.spyOn(
  ...
  it('devrait déterminer le bon type de contrat en fonction des fonctionnalités', () => {
    ...
    const determineContractType = (tokenDeploymentService as any).determineContractType.bind(tokenDeploymentService);

    // Tester différentes combinaisons de fonctionnalités
    expect(determineContractType({
      burnable: false,
      mintable: false,
      pausable: false,
      taxable: false,
  ...
  it('devrait vérifier si un token est déployé', async () => {
    ...
    vi.spyOn(mockBlockchainService.getProvider(), 'getCode').mockImplementation(getCodeSpy);
    ...
    vi.spyOn(mockBlockchainService, 'getProvider').mockReturnValue(mockProvider);
  ...
  it('devrait récupérer l\'historique des déploiements', async () => {
    ...
    expect(history[0].contractAddress).toBe('0x1234567890123456789012345678901234567890');
