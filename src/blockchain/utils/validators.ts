 ...
 * @returns true si l'adresse est valide, false sinon
 */
export const validateAddress = (address: string, chainName: string): boolean => {
 ...
 */
export const validateTokenConfig = (config: TokenConfig): { valid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  } else if (typeof config.initialSupply === 'number' && config.initialSupply <= 0) {
  ...
  if (config.maxSupply !== undefined) {
    ...
    if (
      (typeof config.initialSupply === 'number' && typeof config.maxSupply === 'number' && config.initialSupply > config.maxSupply) ||
      (typeof config.initialSupply === 'bigint' && typeof config.maxSupply === 'bigint' && config.initialSupply > config.maxSupply)
    ) {
  ...
  if (config.antiWhale?.enabled) {
    if (config.antiWhale.maxTransferPercent !== undefined) {
      if (config.antiWhale.maxTransferPercent <= 0 || config.antiWhale.maxTransferPercent > 100) {
  ...
  if (config.taxable?.enabled) {
    if (config.taxable.buyTaxPercent !== undefined) {
      if (config.taxable.buyTaxPercent < 0 || config.taxable.buyTaxPercent > 25) {
        errors.push('Buy tax percent must be between 0 and 25');
      }
    }
    if (config.taxable.sellTaxPercent !== undefined) {
      if (config.taxable.sellTaxPercent < 0 || config.taxable.sellTaxPercent > 25) {
        errors.push('Sell tax percent must be between 0 and 25');
      }
    }
    if (config.taxable.transferTaxPercent !== undefined) {
      if (config.taxable.transferTaxPercent < 0 || config.taxable.transferTaxPercent > 25) {
