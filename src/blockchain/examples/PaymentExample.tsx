...
const PaymentExample: React.FC = () => {
  ...
  const [selectedCurrency, setSelectedCurrency] = useState<CryptocurrencyInfo | null>(null);
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      ...
      alert('Veuillez installer MetaMask ou un autre wallet compatible EIP-1193');
    }
  };
    ...
    <div className="payment-example">
      ...
      <div className="wallet-connection">
        ...
        {!walletConnected ? (
          ...
          <div>
            <p>Wallet connecté: {walletAddress.substring(0, 6)}...{walletAddress.substring(walletAddress.length - 4)}</p>
          </div>
        )}
      </div>
      ...
      <div className="currency-selection">
        ...
        <div className="currency-list">
          {supportedCurrencies.map((currency) => (
            <div 
              key={currency.symbol} 
              className={`currency-item ${selectedCurrency?.symbol === currency.symbol ? 'selected' : ''}`}
              onClick={() => handleCurrencySelect(currency)}
            >
              <img src={currency.logoUrl} alt={currency.name} width="24" height="24" />
              <span>{currency.name} ({currency.symbol})</span>
              {currency.isNative && <span className="badge native">Native</span>}
              {currency.isStablecoin && <span className="badge stablecoin">Stablecoin</span>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="amount-selection">
        ...
        {cryptoAmount && selectedCurrency && (
          <div className="crypto-amount">
            <p>Montant en crypto: <strong>{cryptoAmount.formatted}</strong></p>
            <p>Valeur: <strong>{cryptoAmount.valueEUR.toFixed(2)} EUR</strong></p>
          </div>
        )}
        ...
        {estimatedFees && (
          <div className="fees">
            <h4>Frais estimés:</h4>
            <p>Base: <strong>{estimatedFees.baseFee.formatted}</strong> ({estimatedFees.baseFee.valueEUR.toFixed(2)} EUR)</p>
            <p>Max: <strong>{estimatedFees.maxFee.formatted}</strong> ({estimatedFees.maxFee.valueEUR.toFixed(2)} EUR)</p>
            <p>Temps estimé: <strong>{estimatedFees.estimatedTimeSeconds}</strong> secondes</p>
          </div>
        )}
      </div>
      ...
      <div className="payment-actions">
        ...
        {session && (
          <div className="payment-session">
            ...
            <p>Statut: <strong>{formatPaymentStatus(paymentStatus)}</strong></p>
            <p>Adresse de réception: <strong>{session.receivingAddress}</strong></p>
            <p>Montant dû: <strong>{session.amountDue.formatted}</strong></p>
            <p>Expire le: <strong>{formatExpiryDate(session.expiresAt)}</strong></p>
            ...
            <div className="payment-status-actions">
              ...
              <div className="tx-hash-input">
                ...
                <button 
                  onClick={() => {
                    const txHashInput = document.getElementById('txHash') as HTMLInputElement;
