...
export const usePayment = (chainName: string, walletProvider?: any) => {
  ...
  const convertEURtoCrypto = useCallback(
    async (amountEUR: number, currencySymbol: string) => {
      setError(null);
      try {
        const paymentService = getPaymentService();
        const amount = await paymentService.convertEURtoCrypto(
          amountEUR,
          currencySymbol
        );
  ...
  const estimateTransactionFees = useCallback(
    async (amount: number, currencyAddress: string | null) => {
      setError(null);
      try {
        const paymentService = getPaymentService();
  ...
  const initPaymentSession = useCallback(
    async (params: PaymentInitParams) => {
      setError(null);
  ...
  const confirmPayment = useCallback(
    async (sessionId: string, txHash: string) => {
      setError(null);
      setIsProcessing(true);
      try {
        const paymentService = getPaymentService();
  ...
  const createPaymentSession = useCallback(
    async (amount: bigint, currency: string) => {
      setError(null);
      setIsProcessing(true);
      try {
        const paymentService = getPaymentService();
