...
export const useTokenDeployment = (chainName: string, walletProvider?: any) => {
  ...
  // Déployer un token
  const deployToken = useCallback(async (tokenConfig: TokenConfig): Promise<DeploymentResult | null> => {
    setIsDeploying(true);
    ...
    try {
      ...
      if (!validationResult.valid) {
        throw new Error(`Invalid token configuration: ${validationResult.errors.join(', ')}`);
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error during token deployment';
  ...
  const setupAutoLiquidity = useCallback(async (
    tokenAddress: string,
    ...
    if (!tokenAddress) {
      setDeploymentError('Token address is required for auto-liquidity setup');
      return false;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error during auto-liquidity setup';
      setDeploymentError(errorMessage);
  ...
  const checkDeploymentStatus = useCallback(async (txHash: string): Promise<boolean> => {
    if (!txHash) {
      setDeploymentError('Transaction hash is required to check deployment status');
      return false;
    }
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error checking deployment status';
      setDeploymentError(errorMessage);
      return false;
    }
  }, []);
