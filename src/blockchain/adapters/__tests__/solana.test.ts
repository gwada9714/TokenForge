...
import { SolanaBlockchainService, SolanaPaymentService, SolanaTokenService } from '../solana';
import { mockSolanaConnection, mockSolanaWallet, mockSplToken } from '../../../tests/mocks/solana';
...
vi.mock('@solana/spl-token', () => ({
    ...
    getOrCreateAssociatedTokenAccount: vi.fn().mockImplementation(() => mockSplToken.getOrCreateAssociatedTokenAccount()),
...
describe('SolanaBlockchainService', () => {
    ...
    it('should get balance correctly', async () => {
        const balance = await service.getBalance('5n7pbXHbJaGkh3hJLKXCS4ht3JqJnWoSJfS7cXW3KJeJ');
        expect(mockSolanaConnection.getBalance).toHaveBeenCalledWith(expect.any(PublicKey));
    ...
    it('should estimate gas correctly', async () => {
        ...
        expect(mockSolanaConnection.simulateTransaction).toHaveBeenCalledWith(transaction);
    ...
    it('should get minimum balance for rent exemption correctly', async () => {
        ...
        expect(mockSolanaConnection.getMinimumBalanceForRentExemption).toHaveBeenCalledWith(100);
...
describe('SolanaPaymentService', () => {
    ...
    beforeEach(() => {
        ...
        service.blockchainService = {
            ...
            getRecentBlockhash: vi.fn().mockResolvedValue('5n7pbXHbJaGkh3hJLKXCS4ht3JqJnWoSJfS7cXW3KJeJ')
        };
    });
    ...
    it('should verify payment correctly for valid transaction', async () => {
        ...
        expect(mockSolanaConnection.getSignatureStatus).toHaveBeenCalledWith('validSignature');
    ...
    it('should verify payment correctly for invalid transaction', async () => {
        ...
        expect(mockSolanaConnection.getSignatureStatus).toHaveBeenCalledWith('invalidSignature');
    ...
    it('should transfer SOL correctly', async () => {
        ...
        const result = await service.transferSOL(keypair, '5n7pbXHbJaGkh3hJLKXCS4ht3JqJnWoSJfS7cXW3KJeJ', 1000000000);
...
describe('SolanaTokenService', () => {
    ...
    it('should invalidate token config with decimals > 9', () => {
        ...
        const result = service.validateTokenConfig(invalidConfig);
        expect(result.valid).toBe(false);
        expect(result.errors).toContain('Decimals should be between 0 and 9 for Solana tokens');
    });
    ...
    it('should estimate deployment cost correctly', async () => {
        ...
        mockSolanaConnection.getMinimumBalanceForRentExemption = vi.fn().mockResolvedValue(2039280);
        ...
        expect(mockSolanaConnection.getMinimumBalanceForRentExemption).toHaveBeenCalledTimes(2);
    ...
    it('should setup auto liquidity correctly', async () => {
        const result = await service.setupAutoLiquidity('TokenAddressMock123456789', {
            pairWith: 'SOL',
