...
import { AvalancheBlockchainService, AvalanchePaymentService, AvalancheTokenService } from '../avalanche';
...
describe('AvalancheBlockchainService', () => {
    let service: AvalancheBlockchainService;
    ...
    beforeEach(() => {
        ...
        service = new AvalancheBlockchainService({});
    ...
    it('should get network id correctly', async () => {
        ...
        expect(networkId).toBe(1); // Mockée à 1, mais serait 43114 pour Avalanche C-Chain
    });
    ...
    it('should handle Avalanche specific methods', async () => {
        // Test pour les méthodes spécifiques à Avalanche si elles existent
        // Par exemple, si l'adaptateur Avalanche a une méthode pour interagir avec les X-Chain ou P-Chain
        expect(service).toBeInstanceOf(AvalancheBlockchainService);
    });
});

describe('AvalanchePaymentService', () => {
    let service: AvalanchePaymentService;
    ...
    beforeEach(() => {
        ...
        service = new AvalanchePaymentService({});
    ...
    it('should create payment session with correct id format', async () => {
        ...
        const sessionId = await service.createPaymentSession(1000000000000000000n, 'AVAX');
        expect(sessionId).toMatch(/^avax-\d+-\d+$/);
        expect(sessionId).toContain('avax-1735689600000-500');
...
describe('AvalancheTokenService', () => {
    let service: AvalancheTokenService;

    beforeEach(() => {
        service = new AvalancheTokenService({});
        ...
        service.blockchainService = {
            ...
            getNetworkId: vi.fn().mockResolvedValue(43114)
        };
    });
    ...
    it('should handle Avalanche specific token features', () => {
        // Test pour les fonctionnalités spécifiques aux tokens sur Avalanche
        const avaxConfig = {
            name: 'Avalanche Token',
            symbol: 'AVAX',
            decimals: 18,
            initialSupply: 1000000,
            // Propriétés spécifiques à Avalanche si nécessaire
            burnable: true,
            mintable: true
        };

        const result = service.validateTokenConfig(avaxConfig);
        expect(result.valid).toBe(true);
    });
});
