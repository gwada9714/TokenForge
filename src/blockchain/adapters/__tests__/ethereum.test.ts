...
describe("EthereumPaymentService", () => {
  ...
  it("should verify payment correctly for valid transaction", async () => {
    ...
    console.log(
      "mockPublicClient.getTransaction mock calls:",
      mockPublicClient.getTransaction.mock?.calls
    );
  ...
  it("should verify payment correctly for invalid transaction", async () => {
    // Réinitialiser et configurer le mock pour ce test spécifique
    console.log(
      "Test: should verify payment correctly for invalid transaction"
    );
    console.log("Configuring mock for getTransaction (invalid case)...");

    mockPublicClient.getTransaction = vi.fn().mockImplementation((params) => {
      console.log(
        "Mock getTransaction called with params (invalid case):",
        params
      );
    ...
    console.log(
      "Checking if mockPublicClient.getTransaction was called for invalid case..."
    );
    console.log(
      "mockPublicClient.getTransaction mock calls (invalid case):",
      mockPublicClient.getTransaction.mock?.calls
    );
...
describe("EthereumTokenService", () => {
  ...
  it("should deploy token correctly", async () => {
    ...
    console.log(
      "Provider injected, service after injection:",
    ...
    try {
      ...
      console.log(
        "mockWalletClient.getAddresses mock calls:",
        mockWalletClient.getAddresses.mock?.calls
      );

      console.log("Checking if mockWalletClient.deployContract was called...");
      console.log(
        "mockWalletClient.deployContract mock calls:",
        mockWalletClient.deployContract.mock?.calls
      );
      ...
      expect(result.transactionHash).toBe(
        mockWalletClient.deployContract.mock.results[0].value
      );
    } catch (error) {
