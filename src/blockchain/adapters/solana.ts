  ...
  async calculateFees(amount: bigint): Promise<bigint> {
    ...
    try {
      ...
      if (amount > 100000000n) {
        // Si montant > 0.1 SOL, on suppose une transaction complexe
...
export class SolanaTokenService implements ITokenService {
  ...
  async deployToken(tokenConfig: TokenConfig): Promise<DeploymentResult> {
    try {
      ...
      // Mint initial supply
      const initialSupply =
        tokenConfig.initialSupply * 10 ** tokenConfig.decimals;
      await mintTo(
        connection,
        payer,
        tokenMint,
      ...
      const transactionHash =
        signatures[0]?.signature || "unknown_transaction_hash";
  ...
  async estimateDeploymentCost(tokenConfig: TokenConfig): Promise<bigint> {
    try {
      ...
      const mintRentExemption =
        await connection.getMinimumBalanceForRentExemption(mintAccountSize);
      ...
      // Coût total
      const totalCost =
        mintRentExemption + tokenAccountRentExemption + transactionFees;
  ...
  validateTokenConfig(tokenConfig: TokenConfig): ValidationResult {
    ...
    if (
      tokenConfig.decimals !== undefined &&
      (tokenConfig.decimals < 0 || tokenConfig.decimals > 9)
    ) {
