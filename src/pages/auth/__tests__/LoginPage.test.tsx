...
vi.mock("react-router-dom", () => ({
  ...
  Link: ({
    ...
    [key: string]: any;
  }) => <a {...props}>{children}</a>, // Mock de Link
}));
...
describe("LoginPage", () => {
  ...
  it("renders login form correctly", () => {
    ...
    expect(
      screen.getAllByRole("textbox", { name: /email address/i }).length
    ).toBe(1);
    expect(screen.getAllByRole("textbox", { name: /password/i }).length).toBe(
      1
    );
    expect(
  ...
  it("displays error message when login fails", () => {
    ...
    const mockError: AuthError = new AuthError(
