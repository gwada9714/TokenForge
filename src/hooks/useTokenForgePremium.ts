...
export const useTokenForgePremium = () => {
  ...
  const [userSubscriptions, setUserSubscriptions] = useState<Record<string, ServiceSubscription>>({});
  const [isLoading, setIsLoading] = useState(true);

  const premiumServicesAddress = getContractAddress('PREMIUM_SERVICES', chain?.id || 1);
  ...
  const calculateServiceCost = useCallback((serviceId: string, months: number) => {
  ...
  // Souscription à un service
  const subscribeToService = useCallback(async (serviceId: string, months: number) => {
    try {
  ...
  useEffect(() => {
    if (servicesData) {
      const formattedServices: PremiumService[] = (servicesData as any[]).map(service => ({
        id: service.id,
        name: service.name,
    ...
    const fetchUserSubscriptions = async () => {
      ...
      for (const service of services) {
        try {
          ...
          const [startTime, endTime, isActive] = data as [bigint, bigint, boolean];
          ...
          console.error(`Erreur lors de la récupération de l'abonnement pour ${service.id}:`, error);
        }
      }
