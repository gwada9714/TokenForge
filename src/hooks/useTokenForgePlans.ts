...
export const useTokenForgePlans = () => {
    ...
    async (planType: PlanType) => {
      try {
        ...
        if (balance && balance.value < priceInWei) {
          throw new Error(
            `Solde insuffisant. Vous avez besoin de ${formatPlanPrice(
              planType
            )}`
          );
        }
        ...
        if (error.code === 4001) {
          ...
          throw new Error(
            "Erreur de transaction. Vérifiez votre solde et les frais de gas"
          );
        } else if (error.code === "INSUFFICIENT_FUNDS") {
          throw new Error(
            "Solde insuffisant pour couvrir le prix et les frais de gas"
          );
  ...
  const isLoading =
    isProcessing || isContractLoading || isConfirming || isReadLoading;
