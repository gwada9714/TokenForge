...
export const useUserTokens = (chainId?: number) => {
  ...
  useEffect(() => {
    const fetchTokensData = async () => {
      if (!tokenAddresses || !Array.isArray(tokenAddresses) || !address || !network || !publicClient) {
        setTokens([]);
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const tokenPromises = tokenAddresses.map(async (tokenAddress: Address) => {
          ...
          const tokenData: TokenData = {
            ...
            network,
            createdAt: new Date((creationData as { timestamp: number }).timestamp * 1000),
            features: {
            ...
            taxConfig: {
              ...
              transferTax: Number(formatUnits((taxConfig as any).transferTax, 2)),
