...
export function useTokenDeploy() {
  ...
  const checkWalletStatus = async (): Promise<boolean> => {
    if (!walletClient) {
      if (isAuthenticated) {
        ...
        setError(
          new Error(
            "Wallet non disponible. Vérifiez votre connexion avec MetaMask."
          )
        );
        return false;
      } else {
        try {
          ...
          setError(
            new Error("Veuillez connecter votre wallet pour déployer un token.")
          );
          return false;
        }
      }
  ...
  const deployToken = async (
    ...
    try {
      ...
      const errorMessage =
        errorObj.message || "Erreur inconnue lors du déploiement";
