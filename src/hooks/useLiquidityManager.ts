...
export const useLiquidityManager = () => {
  ...
  const [userLiquiditySettings, setUserLiquiditySettings] = useState<
    Record<string, LiquiditySettings>
  >({});
  const [liquidityStatus, setLiquidityStatus] = useState<
    Record<string, LiquidityStatus>
  >({});
  ...
  const fetchLiquidityStatus = useCallback(
    async (tokenAddress: string) => {
      ...
      try {
        ...
        const randomVariance =
          Math.random() * 2 * settings.rebalanceThreshold -
          settings.rebalanceThreshold;
        const currentRatio = Math.max(
          0,
          Math.min(100, settings.targetRatio + randomVariance)
        );

        // Simuler une date de dernier rééquilibrage
        const now = new Date();
        const lastRebalance = new Date(now);
        lastRebalance.setHours(
          lastRebalance.getHours() -
            Math.floor(Math.random() * settings.rebalanceInterval)
        );

        // Calculer la prochaine date prévue
        const nextScheduledRebalance = new Date(lastRebalance);
        nextScheduledRebalance.setHours(
          nextScheduledRebalance.getHours() + settings.rebalanceInterval
        );

        // Déterminer si un rééquilibrage est nécessaire
        const needsRebalance =
          Math.abs(currentRatio - settings.targetRatio) >
          settings.rebalanceThreshold;
  ...
  const setupAutomaticLiquidity = useCallback(
    async (tokenAddress: string, settings: LiquiditySettings) => {
      if (!address || !tokenAddress) return;
      ...
      try {
        ...
        const db = await getFirestore();
        const liquiditySettingsCollectionRef = collection(
          db,
          "liquiditySettings"
        );
        ...
        toast.error(
          "Erreur lors de la configuration de la liquidité automatique"
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const modifyLiquiditySettings = useCallback(
    async (tokenAddress: string, settings: LiquiditySettings) => {
      if (!address || !tokenAddress) return;
      ...
      try {
        ...
        toast.error(
          "Erreur lors de la modification des paramètres de liquidité"
        );
      } finally {
        setIsLoading(false);
      }
