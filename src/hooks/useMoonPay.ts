...
interface UseMoonPayResult {
  ...
  checkTransactionStatus: (transactionId: string) => Promise<MoonPayTransaction | null>;
  loading: boolean;
  error: Error | null;
}
...
export const useMoonPay = (): UseMoonPayResult => {
  ...
  const initiatePayment = useCallback(async (amount: string): Promise<MoonPayTransaction> => {
    if (!address) {
    ...
    try {
      ...
      const transaction = await moonpayService.initiateTransaction(address, amount);
      return transaction;
    } catch (err) {
  ...
  const checkTransactionStatus = useCallback(async (transactionId: string): Promise<MoonPayTransaction | null> => {
    if (!transactionId) {
