...
export const useTokenDeployment = () => {
  ...
  const deployToken = useCallback(async (params: TokenDeploymentParams) => {
    ...
    try {
      const { name, symbol, decimals, initialSupply, factoryAddress } = params;
      ...
      const hash = await handleTransaction(async () => {
        const hash = await walletClient.writeContract({
          ...
          args: [name, symbol, decimals, parsedSupply, walletClient.account.address],
        });
      ...
      setState(prev => ({
        ...prev,
        loading: false,
        error: error instanceof Error ? error : new Error('Failed to deploy token'),
      }));
      throw error;
    }
