...
export function useToken(tokenAddress: string) {
  ...
  useEffect(() => {
    if (
      ...
      setError(null);
      setIsLoading(false);
    } else if (nameError || symbolError || totalSupplyError || decimalsError || isMintableError || isBurnableError) {
      setError("Erreur lors du chargement des informations du token");
      setIsLoading(false);
    }
  }, [tokenAddress, name, symbol, totalSupply, decimals, isMintable, isBurnable, nameError, symbolError, totalSupplyError, decimalsError, isMintableError, isBurnableError]);
