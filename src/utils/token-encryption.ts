...
export class TokenEncryption {
  ...
  public async encryptToken(token: string): Promise<{ encryptedData: ArrayBuffer; iv: Uint8Array }> {
    try {
  ...
  public async decryptToken(encryptedData: ArrayBuffer, iv: Uint8Array): Promise<string> {
    try {
      const key = await this.getEncryptionKey();
