...
export class FirebaseDebugger {
  ...
  static getGlobalFirebaseState(): void {
    ...
    logger.log(
      ...
      this.formatObject(firebaseGlobal)
    );

    // Vérifier les dépendances Firebase
    const firebaseDeps = Object.keys(global).filter(
      (key) =>
        key.toLowerCase().includes("firebase") ||
        (typeof (global as any)[key] === "object" &&
          (global as any)[key]?.SDK_VERSION)
    );
    logger.log(
      LogLevel.INFO,
      "Dépendances Firebase disponibles:",
      firebaseDeps
    );
  }
  ...
  static async checkFirebaseState(app: FirebaseApp): Promise<void> {
    try {
      ...
      if (!app.name) {
        throw new Error(
          "L'application Firebase n'est pas correctement initialisée"
        );
      }
      ...
      logger.log(
        LogLevel.ERROR,
        "Erreur lors de la vérification de l'état Firebase:",
        error
      );
      throw error;
    }
  ...
  static async checkAuthState(auth: Auth): Promise<void> {
    try {
      ...
      if (!auth.app) {
        throw new Error(
          "L'authentification n'est pas correctement initialisée"
        );
      }
    } catch (error) {
      logger.log(
        LogLevel.ERROR,
        "Erreur lors de la vérification de l'état Auth:",
        error
      );
      throw error;
    }
  ...
  private static async getLoadedModules(app: FirebaseApp): Promise<string[]> {
    ...
    try {
      ...
      const moduleTests = [
        {
          name: "auth",
          test: async () => (await import("firebase/auth")).getAuth(app),
        },
        {
          name: "firestore",
          test: async () =>
            (await import("firebase/firestore")).getFirestore(app),
        },
        {
          name: "storage",
          test: async () => (await import("firebase/storage")).getStorage(app),
        },
        {
          name: "functions",
          test: async () =>
            (await import("firebase/functions")).getFunctions(app),
        },
      ];
