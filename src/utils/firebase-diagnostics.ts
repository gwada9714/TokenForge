...
export class FirebaseDiagnosticsService {
  ...
  private formatError(error: unknown): string {
    if (error instanceof Error) {
      ...
      return `${firebaseError.name}: ${firebaseError.message} ${
  ...
  async runConnectionTest(): Promise<boolean> {
    try {
      ...
      captureException(
        ...
        { context: "Firebase Connection Test" }
      );
      return false;
    }
  }
  ...
  getServiceStatus(): Record<string, unknown> {
    try {
      ...
      captureException(
        error instanceof Error ? error : new Error(String(error)),
        { context: "Firebase Service Status" }
      );
  ...
  async checkFirebaseHealth(): Promise<FirebaseDiagnostics> {
    ...
    try {
      ...
      try {
        ...
        captureException(
          error instanceof Error ? error : new Error(String(error)),
          { context: "Firebase Auth Diagnostics" }
        );
      }
      ...
      try {
        ...
        if (
          ...
          const config = this.parseEmulatorConfig(
            import.meta.env.VITE_FIREBASE_FIRESTORE_EMULATOR_HOST
          );
          connectFirestoreEmulator(db, config.host, config.port);
        }
        ...
        captureException(
          error instanceof Error ? error : new Error(String(error)),
          { context: "Firebase Firestore Diagnostics" }
        );
      }
      ...
      try {
        ...
        captureException(
          error instanceof Error ? error : new Error(String(error)),
          { context: "Firebase Functions Diagnostics" }
        );
      }
      ...
      try {
        ...
        if (!import.meta.env.DEV && !import.meta.env.TEST) {
          ...
          diagnostics.performance.error =
            "Performance monitoring désactivé en développement";
        }
      } catch (error) {
        diagnostics.performance.error =
          error instanceof Error ? error.message : "Erreur inconnue";
        ...
        captureException(
          error instanceof Error ? error : new Error(String(error)),
          { context: "Firebase Performance Diagnostics" }
        );
      }
