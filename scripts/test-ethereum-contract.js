...
async function main() {
  ...
  try {
    ...
    if (!privateKey) {
      ...
      console.log(
        "Using a random wallet for demonstration (cannot send transactions)"
      );
      ...
      if (process.env.TKN_TOKEN_ADDRESS) {
        console.log(
          "\nFound existing contract address:",
          process.env.TKN_TOKEN_ADDRESS
        );
      ...
      if (Number(retrievedValue) === storeValue) {
        ...
        console.log(
          "Contract interaction failed: retrieved value does not match stored value"
        );
      }
    }
