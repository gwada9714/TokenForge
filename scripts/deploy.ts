...
async function main() {
  ...
  try {
    ...
    const contractPath = path.join(
      __dirname,
      "../artifacts/contracts/TokenForgeFactory.sol/TokenForgeFactory.json"
    );
    ...
    if (process.env.ETHERSCAN_API_KEY) {
      ...
      console.log(
        `npx hardhat verify --network ${process.env.NETWORK} ${contractAddress} ${TKN_TOKEN_ADDRESS} ${TREASURY_ADDRESS} ${TAX_SYSTEM_ADDRESS}`
      );
    }
