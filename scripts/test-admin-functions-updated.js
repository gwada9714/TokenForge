...
async function main() {
    ...
    try {
        ...
        const contract = new ethers.Contract(contractAddress, TOKEN_FORGE_FACTORY_ABI, provider);
        ...
        if (isOwner && wallet) {
            ...
            const contractWithSigner = new ethers.Contract(contractAddress, TOKEN_FORGE_FACTORY_ABI, wallet);
            ...
            console.log("vous devez utiliser un script séparé avec des confirmations de transaction.");
            console.log("Ces opérations modifient l'état de la blockchain et nécessitent des frais de gaz.");
        }
    } catch (error) {
