...
const securityChecks = {
  checkEnvVariables() {
    ...
    if (missingVars.length > 0) {
      console.log(LOG_LEVELS.ERROR, `Missing required environment variables: ${missingVars.join(', ')}`);
      return false;
    }
  ...
  async checkFirebaseConfig() {
    ...
    try {
      ...
      console.log(LOG_LEVELS.ERROR, `Firebase configuration error: ${error.message}`);
      return false;
    }
  },
  ...
  checkCSPConfig() {
    ...
    const checks = {
      ...
      unsafeInline: !content.includes("'unsafe-inline'") || content.includes('process.env.NODE_ENV === "development"')
    };
  ...
  checkSecurityHeaders() {
    ...
    if (missingHeaders.length > 0) {
      console.log(LOG_LEVELS.WARNING, `Missing security headers: ${missingHeaders.join(', ')}`);
      return false;
    }
