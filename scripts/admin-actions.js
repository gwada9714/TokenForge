...
async function main() {
  ...
  console.log(
    `Actions administratives pour le contrat à l'adresse : ${contractAddress}`
  );
  ...
  try {
    ...
    if (!process.env.PRIVATE_KEY) {
      console.error(
        "Erreur: Aucune clé privée trouvée dans les variables d'environnement."
      );
      console.log(
        "Ajoutez votre clé privée dans le fichier .env: PRIVATE_KEY=votre_clé_privée"
      );
      process.exit(1);
    }
