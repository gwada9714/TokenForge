...
async function testPaymentSession() {
  ...
  try {
    ...
    console.error('Erreur lors du test de création de session de paiement:', error);
  }
}
...
async function simulateCreateSession(network, currency, amountEUR) {
  ...
  return {
    ...
    receivingAddress: network === 'solana' ? SOLANA_WALLET_ADDRESS : WALLET_ADDRESS,
    amountDue: {
    ...
    currency: {
      symbol: currency,
      address: currency === 'ETH' || currency === 'SOL' ? null : '0xTokenAddress',
