...
test.describe("Flux de création de token", () => {
  ...
  test("devrait créer un token standard avec succès", async ({ page }) => {
    ...
    await page.waitForSelector("text=Token déployé avec succès", {
      timeout: 30000,
    });
    ...
    await expect(
      page.locator('button:has-text("Voir les détails")')
    ).toBeVisible();
  ...
  test("devrait afficher une erreur en cas d'échec du paiement", async ({
    page,
  }) => {
    // Naviguer vers la page de création de token
    await page.click("text=Créer un Token");
...
test.describe("Marketplace de templates", () => {
  ...
  test("devrait filtrer les templates par catégorie", async ({ page }) => {
    ...
    await expect(page.locator('.template-card:has-text("BASIC")')).toHaveCount(
      0
    );
  });
  ...
  test("devrait acheter un template et l'utiliser", async ({ page }) => {
    ...
    await expect(
      page.locator('h1:has-text("Standard ERC-20 Token")')
    ).toBeVisible();
    ...
    await expect(
      page.locator('input[name="features.transferable"]')
    ).toBeChecked();
