...
exports.verifyMoonPayWebhookSignature = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
        }
        if (!MOONPAY_WEBHOOK_SECRET) {
            throw new functions.https.HttpsError('failed-precondition', 'MoonPay webhook secret is not configured.');
        }
        ...
        throw new functions.https.HttpsError('internal', 'Error verifying webhook signature.');
    }
});
